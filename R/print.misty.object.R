...
#'                               output = FALSE)
#' print(result)
print.misty.object <- function(x, sort.var = x$args$sort.var, digits = x$args$digits,
  ...
  # Check inputs
  .check.input(args = c("digits", "p.digits", "icc.digits", "r.digits", "ess.digits", "mcse.digits"), envir = environment(), input.check = check)

  # Check input 'sort.var'
  if (isTRUE(check)) { if (isTRUE(!is.null(sort.var))) { if (isTRUE(!is.logical(sort.var))) { stop("Please specify TRUE or FALSE for the argument 'sort.var'.", call. = FALSE) } } }
  ...
  # 'horiz' argument
  horiz <- if (isTRUE(is.null(x$args$write))) { TRUE } else { ifelse(isTRUE(!grepl("\\b.txt\\b", x$args$write)), TRUE, FALSE) }
  ...
  switch(x$type, aov.b = {
    ...
    print.object[["test"]] <- rbind(c("Source", "SS", "df", "MSS", "F", "p", "et", "om"), print.object[["test"]])
  ...
  # Repeated Measures Analysis of Varianceaov.w() ------------------------------
    ...
    print.object[["test"]] <- lapply(print.object[["test"]], function(y) rbind(c("Source", "SS", "df", "MSS", "F", "p", "et", "et.p", "om", "om.p"), y))
    ...
    if (isTRUE(getOption("knitr.in.progress"))) {
      ...
      print.object[["epsilon"]][1L, "ep"] <- "epsilon"

      print.object[["test"]][["none"]][1L, "et"] <- "eta2"
      print.object[["test"]][["none"]][1L, "et.p"] <- "eta2p"
      print.object[["test"]][["none"]][1L, "om"] <- "omega2"
      print.object[["test"]][["none"]][1L, "om.p"] <- "omega2p"

      print.object[["test"]][["lb"]][1L, "et"] <- "eta2"
      print.object[["test"]][["lb"]][1L, "et.p"] <- "eta2p"
      print.object[["test"]][["lb"]][1L, "om"] <- "omega2"
      print.object[["test"]][["lb"]][1L, "om.p"] <- "omega2p"

      print.object[["test"]][["gg"]][1L, "et"] <- "eta2"
      print.object[["test"]][["gg"]][1L, "et.p"] <- "eta2p"
      print.object[["test"]][["gg"]][1L, "om"] <- "omega2"
      print.object[["test"]][["gg"]][1L, "om.p"] <- "omega2p"

      print.object[["test"]][["hf"]][1L, "et"] <- "eta2"
      print.object[["test"]][["hf"]][1L, "et.p"] <- "eta2p"
      print.object[["test"]][["hf"]][1L, "om"] <- "omega2"
      print.object[["test"]][["hf"]][1L, "om.p"] <- "omega2p"
      ...
      print.object[["test"]][["none"]][1L, "et"] <- "\u03B7\u00B2"
      print.object[["test"]][["none"]][1L, "et.p"] <- "\u03B7\u00B2p"
      print.object[["test"]][["none"]][1L, "om"] <- "\u03C9\u00B2"
      print.object[["test"]][["none"]][1L, "om.p"] <- "\u03C9\u00B2p"

      print.object[["test"]][["lb"]][1L, "et"] <- "\u03B7\u00B2"
      print.object[["test"]][["lb"]][1L, "et.p"] <- "\u03B7\u00B2p"
      print.object[["test"]][["lb"]][1L, "om"] <- "\u03C9\u00B2"
      print.object[["test"]][["lb"]][1L, "om.p"] <- "\u03C9\u00B2p"

      print.object[["test"]][["gg"]][1L, "et"] <- "\u03B7\u00B2"
      print.object[["test"]][["gg"]][1L, "et.p"] <- "\u03B7\u00B2p"
      print.object[["test"]][["gg"]][1L, "om"] <- "\u03C9\u00B2"
      print.object[["test"]][["gg"]][1L, "om.p"] <- "\u03C9\u00B2p"

      print.object[["test"]][["hf"]][1L, "et"] <- "\u03B7\u00B2"
      print.object[["test"]][["hf"]][1L, "et.p"] <- "\u03B7\u00B2p"
      print.object[["test"]][["hf"]][1L, "om"] <- "\u03C9\u00B2"
      print.object[["test"]][["hf"]][1L, "om.p"] <- "\u03C9\u00B2p"
    ...
    print.object[["test"]][["none"]] <- cbind(c("  Correction", "  None      ", "            ", "            ", "            ", "            "), print.object[["test"]][["none"]])
    print.object[["test"]][["lb"]] <- cbind(c("  Correction ", "  Lower Bound", "             ", "             ", "             ", "             "), print.object[["test"]][["lb"]])
    print.object[["test"]][["gg"]] <- cbind(c("  Correction        ", "  Greenhouse-Geisser", "                    ", "                    ", "                    ", "                    "), print.object[["test"]][["gg"]])
    print.object[["test"]][["hf"]] <- cbind(c("  Correction ", "  Huynh-Feldt", "             ", "             ", "             ", "             "), print.object[["test"]][["hf"]])
    ...
    if (isTRUE("none" %in% print)) {

      .write.table(print.object[["test"]][["none"]], left = 1L, right = 3L, horiz = horiz)

      if (isTRUE(any(print %in% c("GG", "HF", "LB")))) { cat("\n") }

    }

    # Sphericity correction: Greenhouse-Geisser
    if (isTRUE("GG" %in% print)) {

      .write.table(print.object[["test"]][["gg"]], left = 1L, right = 3L, horiz = horiz)

      if (isTRUE(any(print %in% c("HF", "LB")))) { cat("\n") }

    }

    # Sphericity correction: Huynh-Feldt
    if (isTRUE("HF" %in% print)) {

      .write.table(print.object[["test"]][["hf"]], left = 1L, right = 3L, horiz = horiz)

      if (isTRUE(any(print %in% "LB"))) { cat("\n") }

    }

    # Sphericity correction: lower bound
    if (isTRUE("LB" %in% print)) {

      .write.table(print.object[["test"]][["lb"]], left = 1L, right = 3L, horiz = horiz)
    ...
    if (isTRUE(x$args$posthoc)) {
      ...
      cat(paste0("\n  Paired-Samples t-Tests for Multiple Comparison\n\n"))
  ...
  # Bollen-Stine Bootstrap with Incomplete Data, boot.bs() ---------------------
    ...
    cat(" Bollen-Stine Bootstrapping of the Chi-Square Statistic\n\n")

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## Format ####

    # Round
    print.object[, "chisq"] <- formatC(print.object[, "chisq"], digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0"))
    ...
    print.object[, "boot.p"] <- formatC(print.object[, "boot.p"], digits = p.digits, format = "f", zero.print = ifelse(p.digits > 0L, paste0("0.", paste(rep(0L, times = p.digits), collapse = "")), "0"))

    # Column names
    print.object <- rbind(c("R", "nNA", "Chisq", "df", "p", "pBoot"), print.object)

    # Greek letter Chi
    if (is.null(getOption("knitr.in.progress"))) { print.object[1L, ] <- c("R", "nNA", "\U03C7\U00B2", "df", "p", "pBoot") }
    ...
    cat("\n Note. p is the theoretical and pBoot is the bootstrapped p-value.\n")
  ...
  # HC and CR Standard Errors, coeff.robust() ----------------------------------
    ...
    } else if (all(class(x$model) %in% "lme")) {
  ...
  # Correlation Matrix with Statistical Significance Testing, cor.matrix() -----
        ...
        switch(x$args$method,
               ...
               'pearson' = {
                 ...
                 print.object <- data.frame(x = colnames(print.object$cor)[1L],
                                            ...
                                            r = .round(print.object$cor[2L, 1L], digits = digits),
                                            t = .round(print.object$stat[2L, 1L], digits = digits),
                                            df = print.object$df[2L, 1L],
                                            p = .round(print.object$p[2L, 1L], digits = p.digits))

               # Spearman's rank-order correlation coefficient
               }, 'spearman' = {

                 # Asymptotic  test
                 if (isTRUE(!x$args$exact)) {
               ...
               # Kendall's Tau-b correlation coefficient
                 ...
                 if (isTRUE(!x$args$exact)) {
                   ...
                   print.object <- data.frame(x = colnames(print.object$cor)[1L],
                                              y = colnames(print.object$cor)[2L],
                                              n = print.object$n[2L, 1L],
                                              tau = .round(print.object$cor[2L, 1L], digits = digits),
                                              z = .round(print.object$stat[2L, 1L], digits = digits),
                                              p = .round(print.object$p[2L, 1L], digits = p.digits))
        ...
        switch (x$args$method,
                ...
                'pearson' = {
                  ...
                  print.object <- data.frame(Group = c(sort(unique(x$data$.group))[1L], sort(unique(x$data$.group))[2L]),
                                             ...
                                             r = .round(c(print.object$cor[2L, 1L], print.object$cor[1L, 2L]), digits = digits),
                                             t = .round(c(print.object$stat[2L, 1L], print.object$stat[1L, 2L]), digits = digits),
                                             df = c(print.object$df[2L, 1L], print.object$df[1L, 2L]),
                                             p = .round(c(print.object$p[2L, 1L], print.object$p[1L, 2L]), digits = p.digits))
                ...
                # Kendall's Tau-b correlation coefficient
                  ...
                  if (isTRUE(!x$args$exact)) {
                    ...
                    print.object <- data.frame(Group = c(sort(unique(x$data$.group))[1L], sort(unique(x$data$.group))[2L]),
                                               ...
                                               y = colnames(print.object$cor)[2L],
                                               n = c(print.object$n[2L, 1L], print.object$n[1L, 2L]),
                                               tau = .round(c(print.object$cor[2L, 1L], print.object$cor[1L, 2L]), digits = digits),
                                               z = .round(c(print.object$stat[2L, 1L], print.object$stat[1L, 2L]), digits = digits),
                                               p = .round(c(print.object$p[2L, 1L], print.object$p[1L, 2L]), digits = p.digits))
    ...
    ## More than Two Variables ####
      ...
      if (isTRUE("cor" %in% print)) {
        ...
        print.object$cor <- .round(print.object$cor, digits = digits)
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {

          # Lower triangular
          if (isTRUE(x$args$tri %in% c("lower", "both"))) { print.object$cor[-1L, -1L][lower.tri(print.object$cor[-1L, -1L])][which(x$result$p[lower.tri(x$result$p)] <= x$args$alpha)] <- misty::chr.color(print.object$cor[-1L, -1L][lower.tri(print.object$cor[-1L, -1L])][which(x$result$p[lower.tri(x$result$p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE) }

          # Upper triangular
          if (isTRUE(x$args$tri %in% c("upper", "both"))) { print.object$cor[-1L, -1L][upper.tri(print.object$cor[-1L, -1L])][which(x$result$p[upper.tri(x$result$p)] <= x$args$alpha)] <- misty::chr.color(print.object$cor[-1L, -1L][upper.tri(print.object$cor[-1L, -1L])][which(x$result$p[upper.tri(x$result$p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE) }
        ...
        print.object$cor <- apply(print.object$cor, 2L, function(y) gsub("NA", "  ", y))
      ...
      if (isTRUE("n" %in% print & attr(x$data, "missing") && isTRUE(!x$args$na.omit))) {
        ...
        print.object$n <- .round(print.object$n, digits = 0L)
      ...
      if (isTRUE("stat" %in% print)) {
        ...
        print.object$stat <- .round(print.object$stat, digits = digits)
        ...
        print.object$stat <- apply(print.object$stat, 2L, function(y) gsub("NA", "  ", y))
      ...
      if (isTRUE("df" %in% print && "df" %in% names(print.object))) {

        #···················
        #### Round ####

        print.object$df <- .round(print.object$df, digits = 0L)
      ...
      if (isTRUE("p" %in% print)) {
        ...
        print.object$p <- .round(print.object$p, digits = p.digits)
        ...
        print.object$p <- apply(print.object$p, 2L, function(y) gsub("NA", "  ", y))
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) { cat(paste0("       Statistically significant coefficients at \U03B1 = ", signif(x$args$alpha, digits = 2L), " are highlighted ", misty::chr.color(x$args$color, color = x$args$color, bg = "none", style = x$args$style, check = FALSE), "\n")) }
  ...
  # Cross Tabulation, crosstab() -----------------------------------------------
    ...
    if (isTRUE(ncol(x$data) == 2L)) {
      ...
      if (!isTRUE(x$args$freq)) { restab <- restab[-grep("Freq", restab[, 2L]), ] }
      ...
      #### Frequencies and Percentages ####
        ...
        # Format percentages
        restab[grep("%", restab[, 2L]), -c(1L:2L)] <- apply(restab[grep("%", restab[, 2L]), -c(1L:2L)], 2L, function(y) paste0(format(formatC(as.numeric(y), digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), justify = "right"), "%"))
      ...
      ### Output Table Split ####
        ...
        if (isTRUE(x$args$freq)) {
          ...
          .write.table(restab.abs, left = 3L, right = 5L, line = 2L, horiz = horiz)
        ...
        if (isTRUE("row" %in% print)) {
          ...
          .write.table(restab.row, left = 3L, right = 5L, line = 2L, horiz = horiz)
        ...
        if (isTRUE("col" %in% print)) {
          ...
          if (isTRUE(x$args$freq) || isTRUE("row" %in% print)) { cat("\n") }
          ...
          .write.table(restab.col, left = 2L, right = 4L, line = 2L, horiz = horiz)
        ...
        if (isTRUE("total" %in% print)) {
          ...
          if (isTRUE(x$args$freq) || isTRUE("row" %in% print) || isTRUE("col" %in% print)) { cat("\n") }
          ...
          .write.table(restab.total, left = 2L, right = 4L, line = 2L, horiz = horiz)
    ...
    ## Three-Dimensional Matrix ####
      ...
      # No absolute frequencies
      if (!isTRUE(x$args$freq)) { restab <- restab[-grep("Freq", restab[, 3L]), ] }
      ...
      #### Output Table Split ####
        ...
        if (isTRUE(x$args$freq)) {
          ...
          .write.table(restab.abs, left = 2L, right = 4L, line = 2L, horiz = horiz)
        ...
        if (isTRUE("row" %in% print)) {
          ...
          .write.table(restab.row, left = 2L, right = 4L, line = 2L, horiz = horiz)
  ...
  # Frequency Table, freq() ----------------------------------------------------
    ...
    if (isTRUE(ncol(as.data.frame(x$data)) == 1L || (x$args$split && ncol(x$data) == 1L))) {
      ...
      if (isTRUE(!x$args$val.col)) {
        ...
        if (isTRUE(length(print) == 1L && print == "v.perc" && isTRUE(!x$args$freq))) {
        ...
        if (!isTRUE(x$args$freq)) { print.object <- print.object[, -grep("Freq", colnames(print.object))] }
      ...
      ### Values in Columns ####
        ...
        if (!isTRUE(x$args$freq)) { print.object <- print.object[-grep("Freq", print.object[, 1L]), ] }
  ...
  # Coefficient Alpha, item.alpha() --------------------------------------------
    ...
    if (isTRUE("alpha" %in% print)) {
      ...
      print.object$alpha$n.items <- format(print.object$alpha$n.items, justify = "right")

      #—————————————————————————————————————— #
      ### Round ####

      print.object$alpha[, c("alpha", "low", "upp")] <- .round(print.object$alpha[, c("alpha", "low", "upp")], digits = r.digits)

      #—————————————————————————————————————— #
      ### Variable Names ####

      print.object$alpha <- rbind(c("n", "nNA", "nItems", "Alpha", "Low", "Upp"), print.object$alpha)
      ...
      #···················
      #### Sample Size for Pairwise Deletion ####

      if (isTRUE(any(is.na(x$data)) && x$args$missing == "pairwise")) { cat(paste0("\n Note. n = ", paste(range(apply(combn(seq_along(x$data), m = 2L), 2L, function(y) sum(complete.cases(x$data[y[1L]], x$data[y[2L]])))), collapse = "-"), ", Pairwise Deletion\n")) }

      #···················
      #### Residual Covariances ####

      if (isTRUE(!is.null(x$args$rescov))) {

        paste(sapply(x$args$rescov, function(y) paste0("(", paste(y[1L], y[2L], sep = ", "), ")")), collapse = ", ") |>
          (\(p) if (isTRUE(any(is.na(x$data)) && x$args$missing == "pairwise")) {

            cat(paste0("       ", ifelse(length(x$args$rescov) == 1L, "Residual Covariance: ", "Residual Covariances: "), p), "\n")

          } else {

            cat(paste0("\n Note. ", ifelse(length(x$args$rescov) == 1L, "Residual Covariance: ", "Residual Covariances: "), p), "\n")
    ...
    if (isTRUE("item" %in% print)) {
      ...
      if (isTRUE(any(print.object$itemstat$pNA != 0L))) { print.object$itemstat$pNA <- paste0(.round(print.object$itemstat$pNA, digits = digits), "%") }

      #—————————————————————————————————————— #
      ### Round ####

      print.object$itemstat$m <- .round(print.object$itemstat$m, digits = digits)
      print.object$itemstat$sd <- .round(print.object$itemstat$sd, digits = digits)
      print.object$itemstat$min <- .round(print.object$itemstat$min, digits = digits)
      print.object$itemstat$max <- .round(print.object$itemstat$max, digits = digits)

      if (isTRUE("r" %in% colnames(print.object$itemstat))) {

        print.object$itemstat$r <- .round(print.object$itemstat$r, digits = r.digits)

      } else {

        print.object$itemstat$std.ld <- .round(print.object$itemstat$std.ld, digits = r.digits)

      }

      print.object$itemstat$alpha <- .round(print.object$itemstat$alpha, digits = r.digits)
      print.object$itemstat$d.alpha <- .round(print.object$itemstat$d.alpha, digits = r.digits)
      ...
      if (isTRUE("r" %in% colnames(print.object$itemstat))) {
        ...
        print.object$itemstat <- rbind(c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max", "r", "Alpha", "dAlpha"), print.object$itemstat)

      } else {

        print.object$itemstat <- rbind(c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max", "Std.Ld", "Alpha", "dAlpha"), print.object$itemstat)
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")))) {
        ...
        print.object$itemstat[1L, "alpha"] <- sub("Alpha", "\U03B1", print.object$itemstat[1, "alpha"])
        print.object$itemstat[1L, "d.alpha"] <- sub("dAlpha", "\U0394\U03B1", print.object$itemstat[1, "d.alpha"])

      }

      #—————————————————————————————————————— #
      ### Justify and Add Blank Space ####
      ...
      # Add blank space
      print.object$itemstat[, "item"] <- c(print.object$itemstat[1L, "item"],  paste0(" ", print.object$itemstat[-1L, "item"]))
      print.object$itemstat[, "item"] <- format(c(print.object$itemstat[1L, "item"], misty::chr.trim(print.object$itemstat[-1L, "item"], side = "right")), justify = "left")
      ...
      #···················
      #### Output with Item-Total Correlation ####

      if (isTRUE("r" %in% colnames(print.object$itemstat))) {

        if (any(!is.na(x$result$itemstat$alpha))) {

          cat(paste0(ifelse(x$args$correct, " Corrected", ""), " Item-Total ", ifelse(x$args$ordered, "Polyserial ", ""), "Correlation and ", ifelse(!x$args$ordered, "Coefficient Alpha ", "Ordinal Coefficient Alpha "), "if Item Deleted\n\n"))

        } else {

          cat(paste0(ifelse(x$args$correct, " Corrected", "")," Item-Total Correlation\n\n"))

        }

      #···················
      #### Output with Standardized Factor Loading ####

      } else {

        if (any(!is.na(x$result$itemstat$alpha))) {

          cat(paste0(" Standardized Factor Loading and ", ifelse(!x$args$ordered, "Coefficient Alpha ", "Ordinal Coefficient Alpha "), "if Item Deleted\n\n"))

        } else {

          cat(" Standardized Factor Loading\n\n")
      ...
      if (any(!is.na(x$result$itemstat$alpha))) { .write.table(print.object$itemstat, left = 1L, right = 3L, horiz = horiz) } else { .write.table(print.object$itemstat[, !colnames(print.object$itemstat) %in% c("alpha", "d.alpha")], left = 1L, right = 3L, horiz = horiz) }
  ...
  # Confirmatory factor analysis, item.cfa() -----------------------------------
    ...
    # No results to print
    if (isTRUE(!is.null(print) && length(print) == 0L && x$args$test == "none")) { cat("\n  There are no results to print after excluding \"fit\" from the argument 'print'.") }
    ...
    if (isTRUE("descript" %in% print && (!is.null(print.object$descript) || !is.null(print.object$itemfreq)))) {

      #—————————————————————————————————————— #
      ### Continuous Indicators ####

      if (isTRUE(isFALSE(x$args$ordered))) {
        ...
        print.itemstat[, print.round] <- .round(print.itemstat[, print.round], digits = digits)
        print.itemstat[, "pNA"] <- .round(print.itemstat[, "pNA"], digits = digits - 1L)
      ...
      #—————————————————————————————————————— #
      ### Continuous and Ordered-Categorical Indicators ####
        ...
        #...................
        #### Counts for Ordered-Categorical Variables ####
    ...
    if (isTRUE("fit" %in% print && !is.null(print.object$fit))) {
      ...
      # Degrees of freedom
      print.fit[which(print.fit[, 1L] == "Degrees of freedom"), -1L] <- print.fit[which(print.fit[, 1L] == "Degrees of freedom"), -1L] |> (\(p) unlist(ifelse(as.numeric(p) %% 2L != 0L, .round(p, digits = digits - 1L), p)))()
      ...
      if (isTRUE(ncol(print.fit) == 4L)) {
        ...
        print.fit <- rbind(if (isTRUE(which(print.fit[, 1L] == "Chi-Square Test of Model Fit") > 1L)) { print.fit[1L:((which(print.fit[, 1L] == "Chi-Square Test of Model Fit")) - 1L), ] }, c("", "Standard", "Scaled", ""),
                           print.fit[which(print.fit[, 1L] == "Chi-Square Test of Model Fit"):((which(print.fit[, 1L] == "Incremental Fit Indices")) - 1L), ], if (isTRUE(any(!is.na(print.fit[which(print.fit[, 1L] == "Incremental Fit Indices") |> (\(p) (p + 1L):(p + 2L))(), "robust"])))) { c("", "Standard", "Scaled", "Robust") } else { c("", "Standard", "Scaled", "") },
                           print.fit[which(print.fit[, 1L] == "Incremental Fit Indices"):((which(print.fit[, 1L] == "Absolute Fit Indices")) - 1L), ], if (isTRUE(any(!is.na(print.fit[which(print.fit[, 1L] == "Absolute Fit Indices") |> (\(p) (p + 1L):(p + 5L))(), "robust"])))) { c("", "Standard", "Scaled", "Robust") } else { c("", "Standard", "Scaled", "") },
                           print.fit[which(print.fit[, 1L] == "Absolute Fit Indices"):((which(print.fit[, 1L] == "Coefficient of Determination")) - 1L), ], if (isTRUE(any(!is.na(print.fit[which(print.fit[, 1L] == "Coefficient of Determination") |> (\(p) (p + 1L):(p + 2L))(), "robust"])))) { c("", "Standard", "", "Robust") } else { c("", "Standard", "", "") },
                           print.fit[which(print.fit[, 1L] == "Coefficient of Determination"):nrow(print.fit), ],
                           make.row.names = FALSE)

      } else if (isTRUE(ncol(print.fit) == 3L)) {

        print.fit <- rbind(if (isTRUE(print.fit[1L, 1L] == "Loglikelihood")) { print.fit[1L:((which(print.fit[, 1L] == "Chi-Square Test of Model Fit")) - 1L), ]  }, c("", "Standard", "Scaled"),
                           print.fit[which(print.fit[, 1L] == "Chi-Square Test of Model Fit"):((which(print.fit[, 1L] == "Incremental Fit Indices")) - 1L), ], if (isTRUE(any(!is.na(print.fit[which(print.fit[, 1L] == "Incremental Fit Indices") |> (\(p) (p + 1L):(p + 2L))(), "scaled"])))) { c("", "Standard", "Scaled") } else { c("", "Standard", "") },
                           print.fit[which(print.fit[, 1L] == "Incremental Fit Indices"):((which(print.fit[, 1L] == "Absolute Fit Indices")) - 1L), ], if (isTRUE(any(!is.na(print.fit[which(print.fit[, 1L] == "Absolute Fit Indices") |> (\(p) (p + 1L):(p + 2L))(), "scaled"])))) { c("", "Standard", "Scaled") } else { c("", "Standard", "") },
                           print.fit[which(print.fit[, 1L] == "Absolute Fit Indices"):((which(print.fit[, 1L] == "Coefficient of Determination")) - 1L), ], c("", "Standard", ""),
                           print.fit[which(print.fit[, 1L] == "Coefficient of Determination"):nrow(print.fit), ], make.row.names = FALSE)


      } else if (isTRUE(ncol(print.fit) == 2L)) {

        print.fit <- rbind(if (isTRUE(print.fit[1L, 1L] == "Loglikelihood")) { print.fit[1L:((which(print.fit[, 1L] == "Chi-Square Test of Model Fit")) - 1L), ]  }, c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Chi-Square Test of Model Fit"):((which(print.fit[, 1L] == "Incremental Fit Indices")) - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Incremental Fit Indices"):((which(print.fit[, 1L] == "Absolute Fit Indices")) - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Absolute Fit Indices"):((which(print.fit[, 1L] == "Coefficient of Determination")) - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Coefficient of Determination"):nrow(print.fit), ], make.row.names = FALSE)
      ...
      labels <- c("H0 Value, Specified Model", "Scaling Correction Factor", "H1 Value, Unrestricted Model", "Scaling Correction Factor",
                  ...
                  "Scaling correction factor", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI")
      ...
      #...................
      ### Remove 90 Percent CI Rows ####

      which(misty::chr.trim(print.fit[, 2L]) == "" & misty::chr.trim(print.fit[, 1L]) %in% c("90 Percent CI - lower", "90 Percent CI - upper")) |> (\(p) if (isTRUE(length(p) > 0L)) { print.fit <<- print.fit[-p, ] })()
    ...
    if (isTRUE("est" %in% print && !is.null(print.object$param))) {
      ...
      # digits
      print.param[, setdiff(colnames(print.param), c("param", "lhs", "op", "rhs", "pvalue"))] <- .round(print.param[, setdiff(colnames(print.param), c("param", "lhs", "op", "rhs", "pvalue"))], digits = digits)

      # p.digits
      if (isTRUE("pvalue" %in% colnames(print.param))) { print.param[, "pvalue"] <- .round(as.numeric(print.param[, "pvalue"]), digits = p.digits) }

      #—————————————————————————————————————— #
      ### Convert NA into "" ####

      print.param[grep("~", print.param$rhs), colnames(print.param) %in% c("est", "se", "z", "pvalue", "stdyx")] <- ""
      ...
      if (isTRUE(x$args$se  != "none")) {
        ...
        print.param <- rbind(c("", "", "", "", "Estimate", "StdYX"), print.param)
      ...
      # Heading
      write.table(print.param[1L, 4L:ncol(print.param)], quote = FALSE, row.names = FALSE, col.names = FALSE)

      # Horizontal line
      if (isTRUE(horiz)) { cat(paste(rep(" ", times = 2), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param[1L, 4L:ncol(print.param)])) + length(print.param[1L, 4L:ncol(print.param)]) - 4), collapse = ""), "\n") }

      # Columns
      columns <- setdiff(colnames(print.param), c("param", "lhs", "op"))
      ...
      for (i in unique(print.lv$lhs)) {
        ...
        write.table(print.lv[print.lv$lhs == i, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "latent variable covariance"))) {
        ...
        for (i in unique(print.lv.cov$lhs)) {
          ...
          write.table(print.lv.cov[print.lv.cov$lhs == i, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "residual covariance"))) {
        ...
        for (i in unique(print.res.cov$lhs)) {
          ...
          write.table(print.res.cov[print.res.cov$lhs == i, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "latent mean"))) {
        ...
        if (isTRUE(x$args$se  != "none")) { print.mean[grep("NA", print.mean[, "z"]), c("se", "z", "pvalue", "stdyx")] <- "" }

        if (isTRUE(nrow(print.mean) == 1L)) { cat("\n   Latent Mean\n") } else { cat("\n   Latent Means\n") }

        write.table(print.mean[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "latent variance"))) {
        ...
        if (isTRUE(x$args$se  != "none")) { print.var[grep("NA", print.var[, "z"]), c("se", "z", "pvalue", "stdyx")] <- "" }

        if (isTRUE(nrow(print.var) == 1L)) { cat("\n   Latent Variance\n") } else { cat("\n   Latent Variances\n") }

        write.table(print.var[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "intercept"))) {
        ...
        if (isTRUE(x$args$se  != "none")) { print.inter[grep("NA", print.inter[, "z"]), c("se", "z", "pvalue", "stdyx")] <- "" }

        write.table(print.inter[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "threshold"))) {
        ...
        if (isTRUE(x$args$se  != "none")) { print.thres[grep("NA", print.thres[, "z"]), c("se", "z", "pvalue", "stdyx")] <- "" }

        write.table(print.thres[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "scale"))) {
        ...
        write.table(print.scale[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
      ...
      if (isTRUE(any(print.param$param %in% "residual variance"))) {
        ...
        write.table(print.resid[, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)
    ...
    if (isTRUE("modind" %in% print && !is.null(print.object$modind))) {
      ...
      ### Modification Available ####
        ...
        #### Modification above minimum value
          ...
          # Round
          print.modind[, -c(1L:3L)] <- .round(print.modind[, -c(1L:3L)], digits = digits)
          ...
          if (isTRUE(nrow(print.modind.load) > 0L)) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.modind[1L, ])) + length(print.modind[1L, ]) - 5), collapse = ""), "\n") }
    ...
    if (isTRUE("resid" %in% print && !is.null(print.object$resid))) {
      ...
      ### Residual Correlation Matrix Available ####
        ...
        print.resid <- .round(print.resid, digits = p.digits)
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && x$args$resid.minval != 1L && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {

          print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L][lower.tri(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L])][which(abs(x$result$resid[-nrow(x$result$resid), ][lower.tri(x$result$resid[-nrow(x$result$resid), ])]) >= x$args$resid.minval)] <- misty::chr.color(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L][lower.tri(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L])][which(abs(x$result$resid[-nrow(x$result$resid), ][lower.tri(x$result$resid[-nrow(x$result$resid), ])]) > x$args$resid.minval)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)

          print.resid[nrow(print.resid), -1L][abs(as.numeric(print.resid[nrow(print.resid), -1L])) >= x$args$resid.minval] <- misty::chr.color(print.resid[nrow(print.resid), -1L][abs(as.numeric(print.resid[nrow(print.resid), -1L])) >= x$args$resid.minval], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          cat(paste0("\n  Note. Max absolute value for acceptable observed and model-implied discrepancy: ", misty::chr.color(round(x$args$resid.minval, digits = 2L), color = x$args$color, bg = "none", style = x$args$style, check = FALSE), "\n"))
    ...
    if (isTRUE("opdyke" %in% print && !is.null(print.object$opdyke))) {

      # Extract result table
      print.opdyke <- print.object$opdyke

      cat("\n  Relative Opdyke Distribution Percentile Matrix\n\n")

      #—————————————————————————————————————— #
      ### Relative Opdyke Distribution Percentile Matrix not Available ####

      if (isTRUE(is.null(unlist(print.opdyke)))) {

        cat("   Relative Opdyke distribution percentile matrix is not available.")

      #—————————————————————————————————————— #
      ### Relative Opdyke Distribution Percentile Matrix Available ####
        ...
        print.opdyke <- .round(print.opdyke, digits = p.digits)

        #···················
        #### Lower Triangular ####

        print.opdyke[upper.tri(print.opdyke)] <- ""

        #···················
        #### Column Names ####

        print.opdyke <- rbind(colnames(print.opdyke), print.opdyke)
        print.opdyke <- cbind(rownames(print.opdyke), print.opdyke)

        #···················
        #### Format ####

        # Add blank spaces
        print.opdyke[, 1L] <- paste("  ", print.opdyke[, 1L])

        # Justify
        print.opdyke[, 1L] <- format(paste0(print.opdyke[, 1L], "  "), justify = "left")
        print.opdyke[, -1L] <- apply(print.opdyke[, -1L], 2L, function(y) format(paste0(ifelse(any((nchar(colnames(print.opdyke)[-1L]) - p.digits) < 3L), " ", ""), y), justify = "right"))

        # Replace NA
        print.opdyke[-1L, -1L] <- gsub("NA", "  ", print.opdyke[-1L, -1L])
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          print.opdyke[-1L, -1L][lower.tri(print.opdyke[-1L, -1L])][print.opdyke[-1L, -1L][lower.tri(print.opdyke[-1L, -1L])] |> (\(p) as.numeric(ifelse(misty::chr.trim(p) != "NA", p, NA)))() |> (\(q) which(q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L]))()] <- print.opdyke[-1L, -1L][lower.tri(print.opdyke[-1L, -1L])][print.opdyke[-1L, -1L][lower.tri(print.opdyke[-1L, -1L])] |> (\(p) as.numeric(ifelse(misty::chr.trim(p) != "NA", p, NA)))() |> (\(q) which(q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L]))()] |>
            (\(q) na.omit(q[ifelse(!is.na(q), q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L], NA)]))() |>
            (\(r) ifelse(!is.na(r), misty::chr.color(r, color = x$args$color, bg = "none", style = x$args$style, check = FALSE), NA))()
        ...
        .write.table(print.opdyke, left = 2L, right = 4L, horiz = horiz)
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          cat(paste0("\n  Note. Min/Max percentile acceptably close to the observed correlation: ", misty::chr.color(x$args$opdyke.minmax[1L], color = x$args$color, bg = "none", style = x$args$style, check = FALSE), " and ", misty::chr.color(x$args$opdyke.minmax[2L], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)), "\n")
  ...
  # Dynamic Fit Index Cutoffs, item.dfi() --------------------------------------
    ...
    cat(" Dynamic Fit Index Cutoffs\n")

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## print Argument ####

    .check.input(m.character = list(print = c("all", "summary", "model", "cutoff")), envir = environment(), input.check = TRUE)

    if (isTRUE(length(print) == 1L && "all" %in% print)) { print <- c("summary", "model", "cutoff") }
    ...
    if (isTRUE("summary" %in% print)) {
      ...
      # Include spaces
      print.object$summary[1L, 1L] <- paste0(" ", print.object$summary[1L, 1L])
      print.object$summary[-1L, 1L] <- paste0("  ", print.object$summary[-1L, 1L])

      # Justify left and right
      print.object$summary[, 1L] <- format(print.object$summary[, 1L], justify = "left")
      print.object$summary[, 2L] <- format(print.object$summary[, 2L], justify = "right")

      #—————————————————————————————————————— #
      ### Print ####

      print(print.object$summary, col.names = FALSE, row.names = FALSE, quote = FALSE, right = TRUE, max = 99999L)
    ...
    if (isTRUE("model" %in% print)) {
      ...
      cat(paste0("    ", unlist(strsplit(.chr.round(x$sim.model[[1L]], digits = 2L), "\n")), collapse = "\n"), "\n")

      #—————————————————————————————————————— #
      ### Level 1 ####

      if (isTRUE(length(x$sim.model) >= 2L)) {

        cat("\n  ", names(x$sim.model)[2L], "\n")

        cat("    ...\n")
        cat(unlist(strsplit(x$sim.model[[2L]], "\n")) |> (\(p) paste0("    ", p[!p %in% unlist(strsplit(x$sim.model[[1L]], "\n"))], collapse = "\n"))(), "\n")
      ...
      if (isTRUE(length(x$sim.model) >= 3L)) {

        cat("\n  ", names(x$sim.model)[3L], "\n")

        cat("    ...\n")
        cat(unlist(strsplit(x$sim.model[[3L]], "\n")) |> (\(p) paste0("    ", p[!p %in% unlist(strsplit(x$sim.model[[1L]], "\n"))], collapse = "\n"))(), "\n")
      ...
      if (isTRUE(length(x$sim.model) >= 4L)) {

        cat("\n  ", names(x$sim.model)[4L], "\n")

        cat("    ...\n")
        cat(unlist(strsplit(x$sim.model[[4L]], "\n")) |> (\(p) paste0("    ", p[!p %in% unlist(strsplit(x$sim.model[[1L]], "\n"))], collapse = "\n"))(), "\n")
    ...
    if (isTRUE("cutoff" %in% print)) {

      fit.cutoff <- x$result$fit.cutoff
      ...
      fit.cutoff[, c("cfi", "tli", "rmsea", "srmr")] <- sapply(c("cfi", "tli", "rmsea", "srmr"), function(y) ifelse(!is.na(fit.cutoff[, y]), formatC(fit.cutoff[, y], digits = digits, format = "f",zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), NA))

      #—————————————————————————————————————— #
      ### Percent ####

      fit.cutoff[, c("power.c", "power.t", "power.r", "power.s")] <- sapply(c("power.c", "power.t", "power.r", "power.s"), function(y) paste0(fit.cutoff[, y] * 100L, "%"))
      ...
      fit.cutoff.output <- data.frame(c("Level 0", "Specificity"),
                                      rbind(setNames(fit.cutoff[1L, c("cfi", "tli", "rmsea", "srmr")], nm = c("CFI", "TLI", "RMSEA", "SRMR")),
                                      setNames(fit.cutoff[1L, c("power.c", "power.t", "power.r", "power.s")], nm = c("CFI", "TLI", "RMSEA", "SRMR"))), fix.empty.names = FALSE)
      ...
      if (isTRUE(nrow(fit.cutoff) > 1L)) {
        ...
        for (i in 2L:nrow(fit.cutoff)) {
          ...
          fit.cutoff.output <- rbind(fit.cutoff.output,
                                     c(rep("", times = 5L)),
                                     data.frame(c(rownames(fit.cutoff)[i], "Sensitivity"),
                                                rbind(setNames(fit.cutoff[i, c("cfi", "tli", "rmsea", "srmr")], nm = c("CFI", "TLI", "RMSEA", "SRMR")),
                                                setNames(fit.cutoff[i, c("power.c", "power.t", "power.r", "power.s")], nm = c("CFI", "TLI", "RMSEA", "SRMR"))), fix.empty.names = FALSE))
      ...
      fit.cutoff.output <- rbind(c("", "CFI", "TLI", "RMSEA", "SRMR"), fit.cutoff.output)
      ...
      if (isTRUE(!is.null(x$result$fit.emp))) {
        ...
        fit.emp[c("cfi", "tli", "rmsea", "srmr")] <- sapply(c("cfi", "tli", "rmsea", "srmr"), function(y) ifelse(!is.na(fit.emp[y]), formatC(fit.emp[y], digits = digits, format = "f",zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), NA))

        fit.emp["chisq"] <- formatC(as.numeric(fit.emp["chisq"]), digits = digits - 1L, format = "f", zero.print = ifelse(digits - 1L > 0L, paste0("0.", paste(rep(0L, times = digits - 1L), collapse = "")), "0"))

        #···················
        #### Combine Simulated Cutoffs and Empirical Fit Indices ####

        fit.cutoff.output <- rbind(data.frame(fit.cutoff.output[, 1L], cbind("", fit.cutoff.output[, -1L]), fix.empty.names = FALSE),
                                   setNames(fit.emp[c("chisq", "df", "cfi", "tli", "rmsea", "srmr")], c("Chi2", "df", "CFI", "TLI", "RMSEA", "SRMR")))

        #—————————————————————————————————————— #
        ### Column Names ####

        fit.cutoff.output <- rbind(fit.cutoff.output[-nrow(fit.cutoff.output), ], c("Chi2", "df", "CFI", "TLI", "RMSEA", "SRMR"), fit.cutoff.output[nrow(fit.cutoff.output), ])

        #—————————————————————————————————————— #
        ### Format ####

        # Include spaces
        fit.cutoff.output[, 1L] <- paste0("   ", fit.cutoff.output[, 1L])

        # Justify left and right
        fit.cutoff.output[-c((nrow(fit.cutoff.output) - 1L):nrow(fit.cutoff.output)), 1L] <- format(fit.cutoff.output[-c((nrow(fit.cutoff.output) - 1L):nrow(fit.cutoff.output)), 1L], justify = "left")
        fit.cutoff.output[, -1L] <- format(fit.cutoff.output[, -1L], justify = "right")

        fit.cutoff.output[, 1L] <- formatC(fit.cutoff.output[, 1L])

        #—————————————————————————————————————— #
        ### Split Table ####

        fit.cutoff1 <- fit.cutoff.output[-c(nrow(fit.cutoff.output) - 1L, nrow(fit.cutoff.output)), ]
        fit.cutoff2 <- fit.cutoff.output[c(nrow(fit.cutoff.output) - 1L, nrow(fit.cutoff.output)), ]

        # Greek letter Chi
        if (is.null(getOption("knitr.in.progress"))) { fit.cutoff2[1L, ] <- sub("Chi2", "  \U03C7\U00B2", fit.cutoff2[1L, ]) }

        #—————————————————————————————————————— #
        ### Print ####

        cat("\n  Simulated Fit Index Cutoffs\n\n")

        .write.table(fit.cutoff1, left = 2L, right = 4L, horiz = horiz)

        cat("\n  Empirical Fit Indices\n\n")

        # Mismatch between simulated and fitted model
        if (isTRUE(as.numeric(print.object$summary.empirical[1L, 2]) != x$args$n || print.object$summary.empirical[2L, 2] != x$args$estimator || tolower(print.object$summary.empirical[3L, 2]) != x$args$fit.indices)) {

          # Include spaces
          print.object$summary.empirical[, 1L] <- paste0("   ", print.object$summary.empirical[, 1L])

          # Justify left and right
          print.object$summary.empirical[, 1L] <- format(print.object$summary.empirical[, 1L], justify = "left")
          print.object$summary.empirical[, 2L] <- format(print.object$summary.empirical[, 2L], justify = "right")

          #—————————————————————————————————————— #
          ### Print ####

          write.table(print.object$summary.empirical, col.names = FALSE, row.names = FALSE, quote = FALSE)

          cat("\n")

        }

        .write.table(fit.cutoff2, left = 2L, right = 4L, horiz = horiz)

      #—————————————————————————————————————— #
      ### No Empirical Fit Indices ####
        ...
        # Justify left and right
        fit.cutoff.output[, 1L] <- format(fit.cutoff.output[, 1L], justify = "left")
        fit.cutoff.output[, -1L] <- format(fit.cutoff.output[, -1L], justify = "right")
        ...
        .write.table(fit.cutoff.output, left = 2L, right = 4L, horiz = horiz)
      ...
      if (isTRUE(any(is.na(fit.cutoff[, c("cfi", "tli", "rmsea", "srmr")])))) {

        cat("\n Note. Cutoffs with sensitivity smaller than 50% are suppressed.")
  ...
  # Distractor Analysis for Multiple-Choice Items, item.distract() -------------
    ...
    # Distractor selection frequency, convert into character
    if (isTRUE(any(substr(colnames(print.object), 1L, 1L) == "n"))) { print.object[, substr(colnames(print.object), 1L, 1L) == "n"] <- sapply(print.object[, substr(colnames(print.object), 1L, 1L) == "n"], formatC, format = "d") }

    # Distractor selection percentage
    if (isTRUE(any(substr(colnames(print.object), 1L, 1L) == "p"))) { print.object[, substr(colnames(print.object), 1L, 1L) == "p"] <- .round(print.object[, substr(colnames(print.object), 1L, 1L) == "p"], digits = digits) }

    # Attractor-distractor-total correlation
    if (isTRUE(any(substr(colnames(print.object), 1L, 1L) == "r"))) { print.object[, substr(colnames(print.object), 1L, 1L) == "r"] <- .round(print.object[, substr(colnames(print.object), 1L, 1L) == "r"], digits = r.digits) }

    #—————————————————————————————————————— #
    ### Percentage Sign ####

    if (isTRUE(any(substr(colnames(print.object), 1L, 1L) == "p"))) { print.object[, substr(colnames(print.object), 1L, 1L) == "p"] <- sapply(print.object[, substr(colnames(print.object), 1L, 1L) == "p"], function(y) ifelse(y != "NA", paste0(y, "%"), y)) }

    #—————————————————————————————————————— #
    ### Replace NA ####

    print.object[, setdiff(colnames(print.object), c("item", "n", "nUQ", "key"))] <- sapply(print.object[, setdiff(colnames(print.object), c("item", "n", "nUQ", "key"))], function(y) gsub("NA", "  ", y))

    #—————————————————————————————————————— #
    ### Variable Name ####

    print.object <- rbind(misty::chr.gsub(pattern = c("item", "key"), replacement = c("Item", "Key"), colnames(print.object)), print.object)

    #—————————————————————————————————————— #
    ### Justify Left and Right ####

    print.object[, 1L] <- format(paste("  ", print.object[, 1L]), justify = "left")
    print.object[, -1L] <- apply(print.object[, -1L], 2L, function(y) format(y, justify = "right"))

    # Add blank space
    print.object[, "item"] <- c(print.object[1L, "item"],  paste0(" ", print.object[-1L, "item"]))
    print.object[, "item"] <- format(c(print.object[1L, "item"], misty::chr.trim(print.object[-1L, "item"], side = "right")), justify = "left")

    #—————————————————————————————————————— #
    ### Color Highlighting ####

    # R Markdown not in progress
    if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && any(c("n", "perc", "r") %in% print) && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
      ...
      if (isTRUE("n" %in% print)) {
        ...
        for (i in seq_len(nrow(print.object))[-1L]) {

          if (isTRUE(!is.na(x$key$keypos[i - 1L]))) { print.object[i, substr(colnames(print.object), 1L, 1L) == "n" & !colnames(print.object) %in% c("n", "nUQ")][, x$key$keypos[i - 1L]] <- misty::chr.color(print.object[i, substr(colnames(print.object), 1L, 1L) == "n" & !colnames(print.object) %in% c("n", "nUQ")][, x$key$keypos[i - 1L]], color = x$args$color, bg = "none", style = x$args$style, check = FALSE) }
      ...
      if (isTRUE("perc" %in% print)) {
        ...
        for (i in seq_len(nrow(print.object))[-1L]) {
          ...
          if (isTRUE(!is.na(x$key$keypos[i - 1L]))) { print.object[i, substr(colnames(print.object), 1L, 1L) == "p"][, x$key$keypos[i - 1L]] <- misty::chr.color(print.object[i, substr(colnames(print.object), 1L, 1L) == "p"][, x$key$keypos[i - 1L]], color = x$args$color, bg = "none", style = x$args$style, check = FALSE) }
      ...
      if (isTRUE("r" %in% print)) {
        ...
        for (i in seq_len(nrow(print.object))[-1L]) {
          ...
          if (isTRUE(!is.na(x$key$keypos[i - 1L]))) { print.object[i, substr(colnames(print.object), 1L, 1L) == "r"][, x$key$keypos[i - 1L]] <- misty::chr.color(print.object[i, substr(colnames(print.object), 1L, 1L) == "r"][, x$key$keypos[i - 1L]], color = x$args$color, bg = "none", style = x$args$style, check = FALSE) }
    ...
    print |>
      (\(p) if (isTRUE(any(c("n", "perc") %in% p) && !"r" %in% p)) {

        cat("  Distractor Selection\n\n")

      } else if (isTRUE(all(!c("n", "perc") %in% p) && "r" %in% p)) {

        cat("  ", paste0(ifelse(x$args$correct, "Corrected", ""), " Distractor-Total", ifelse(x$args$method == "pbiser", " Point-Biserial", " Biserial"), " Correlation\n\n"))

      } else if (isTRUE(any(c("n", "perc") %in% p) && "r" %in% p)) {

        cat("  Distractor Selection and", paste0(ifelse(x$args$correct, "Corrected", ""), " Distractor-Total", ifelse(x$args$method == "pbiser", " Point-Biserial", " Biserial"), " Correlation\n\n"))

      })()

    # Print distractor analysis table
    .write.table(print.object, left = 2L, right = 4L, horiz = horiz)
  ...
  # Between-Group and Longitudinal MI Evaluation, item.invar() -----------------
    ...
    # Estimated models
    if (isTRUE(!x$args$lavaan.run && any(c("fit", "est", "modind", "resid") %in% print))) { stop("Measurement models were not estimated given the argument setting lavaan.run = FALSE.", call. = FALSE) }
    ...
    if (isTRUE("summary" %in% print && !is.null(print.object$summary))) {
      ...
      if (isFALSE(x$args$ordered)) { print.summary1 <- print.summary1[-12, ] }
    ...
    if (isTRUE("partial" %in% print && !is.null(x$args$partial))) {
      ...
      if (isTRUE(!x$args$ordered)) {
        ...
        if (isTRUE((!x$args$long && length(unique(na.omit(x$data$.group))) == 2L) || x$args$long)) {
          ...
          # Add space
          print.partial$param[1L] <- paste0(print.partial$param[1L], paste0(rep(" ", times = (max(c(nchar(print.object$summary[, 1L]), nchar(print.object$summary[, 2L]))) - 2L) - nchar(print.partial$param[1L])), collapse = ""), collapse = "")
        ...
        #### Between-Group Measurement Invariance with more than Two Groups ####
          ...
          if (isTRUE(all(misty::chr.trim(as.character(unlist(print.partial[nrow(print.partial), ]))) == ""))) { print.partial <- print.partial[-nrow(print.partial), ] }
    ...
    if (isTRUE("fit" %in% print && !is.null(print.object$fit))) {
      ...
      print.fit.stand <- print.object$fit$stand
      print.fit.scaled <- print.object$fit$scaled
      print.fit.robust <- print.object$fit$robust
      ...
      pos.round.stand <- which(print.fit.stand[, 1L] %in% c("CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "SRMR", "GFI"))
      pos.round.scaled <- which(print.fit.scaled[, 1L] %in% c("CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "SRMR", "GFI"))
      pos.round.robust <- which(print.fit.robust[, 1L] %in% c("CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "SRMR", "GFI"))
      ...
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust[pos.round.robust, -1L] <- sapply(print.fit.robust[pos.round.robust, -1L], function(y) ifelse(!is.na(y), formatC(y, digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), NA)) }
      ...
      if (isTRUE(x$args$estimator != "PML")) {
        ...
        pos.noround.stand <- which(print.fit.stand[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Information Criteria", "Coefficient of Determination", "Degrees of freedom", "P-value", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI"))

      } else {

        pos.noround.stand <- which(print.fit.stand[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "P-value", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI"))

      }

      pos.noround.scaled <- which(print.fit.scaled[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "Degrees of freedom", "P-value", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI"))
      pos.noround.robust <- which(print.fit.robust[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "Degrees of freedom", "P-value", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI"))
      ...
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust[-pos.noround.robust, -1L] <- sapply(print.fit.robust[-pos.noround.robust, -1L], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits - 1L, format = "f", zero.print = ifelse(digits - 1L > 0L, paste0("0.", paste(rep(0L, times = digits - 1L), collapse = "")), "0")), NA)) }
      ...
      pos.noblank.stand <- which(print.fit.stand[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria"))
      pos.noblank.group.stand <- which(print.fit.stand[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "Test statistic", "Degrees of freedom", "P-value", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC"))
      pos.blank.rmsea.stand <- which(print.fit.stand[, 1L] %in% c("90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05"))

      pos.noblank.scaled <- which(print.fit.scaled[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria"))
      pos.noblank.group.scaled <- which(print.fit.scaled[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "Test statistic", "Degrees of freedom", "P-value", "Scaling Correction Factor", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC"))
      pos.blank.rmsea.scaled <- which(print.fit.scaled[, 1L] %in% c("90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05"))

      pos.noblank.robust <- which(print.fit.robust[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria"))
      pos.noblank.group.robust <- which(print.fit.robust[, 1L] %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria", "Test statistic", "Degrees of freedom", "P-value", "Scaling Correction Factor", "CFI", "TLI", "RMSEA", "90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05", "SRMR", "GFI", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC"))
      pos.blank.rmsea.robust <- which(print.fit.robust[, 1L] %in% c("90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05"))
      ...
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust[-pos.noblank.robust, 1L] <- paste("", unlist(print.fit.robust[-pos.noblank.robust, 1L])) }
      ...
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust[-pos.noblank.group.robust, 1L] <- paste("", unlist(print.fit.robust[-pos.noblank.group.robust, 1L])) }
      ...
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust[pos.blank.rmsea.robust, 1L] <- paste("", unlist(print.fit.robust[pos.blank.rmsea.robust, 1L])) }
      ...
      if (isTRUE("standard" %in% x$args$print.fit)) {
        ...
        cat("\n   Standard CFI, TLI, RMSEA, and GFI\n")
    ...
    if (isTRUE("est" %in% print && !is.null(print.object$param))) {
      ...
      if (isTRUE(!x$args$long)) {
        ...
        for (i in misty::chr.omit(unique(print.param$group))) {
          ...
          if (isTRUE(any(print.param[print.param$group == i, "param"] %in% "latent variable"))) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3L), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param[1L, c("rhs", "label", "est", "se", "z", "pvalue", "stdyx")])) + length(print.param[1L, c("rhs", "label", "est", "se", "z", "pvalue", "stdyx")]) - 5L), collapse = ""), "\n") }
      ...
      #### Longitudinal Measurement Invariance ####
        ...
        if (isTRUE(any(print.param[, "param"] %in% "latent variable"))) {
          ...
          # Horizontal line
          if (isTRUE(horiz)) { cat(paste(rep(" ", times = 2), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param[1L, c("rhs", "label", "est", "se", "z", "pvalue", "stdyx")])) + length(print.param[1L, c("rhs", "label", "est", "se", "z", "pvalue", "stdyx")]) - 4), collapse = ""), "\n") }
    ...
    if (isTRUE("modind" %in% print && !is.null(print.object$modind))) {
      ...
      switch(x$args$invar,
             ...
             ### Threshold invariance ####
               ...
               print.score <- print.object$score$thres
      ...
      ### Residual Correlation Matrix Available ####
        ...
        if (isTRUE(!x$args$long)) {
          ...
          if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && x$args$resid.minval != 1L && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
            ...
            for (i in seq_along(print.resid)) {
              ...
              print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])), -1L][lower.tri(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])), -1L])][which(abs(x$result$resid[[x$args$invar]][[i]][-nrow(x$result$resid[[x$args$invar]][[i]]), ][lower.tri(x$result$resid[[x$args$invar]][[i]][-nrow(x$result$resid[[x$args$invar]][[i]]), ])]) >= x$args$resid.minval)] <- misty::chr.color(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])), -1L][lower.tri(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])), -1L])][which(abs(x$result$resid[[x$args$invar]][[i]][-nrow(x$result$resid[[x$args$invar]][[i]]), ][lower.tri(x$result$resid[[x$args$invar]][[i]][-nrow(x$result$resid[[x$args$invar]][[i]]), ])]) >= x$args$resid.minval)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)

              print.resid[[i]][nrow(print.resid[[i]]), -1L][abs(as.numeric(print.resid[[i]][nrow(print.resid[[i]]), -1L])) >= x$args$resid.minval] <- misty::chr.color(print.resid[[i]][nrow(print.resid[[i]]), -1L][abs(as.numeric(print.resid[[i]][nrow(print.resid[[i]]), -1L])) >= x$args$resid.minval], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
        ...
        #### Longitudinal Measurement Invariance ####
          ...
          if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && x$args$resid.minval != 1L && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
            ...
            print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L][lower.tri(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L])][which(abs(x$result$resid[[x$args$invar]][-nrow(x$result$resid[[x$args$invar]]), ][lower.tri(x$result$resid[[x$args$invar]][-nrow(x$result$resid[[x$args$invar]]), ])]) >= x$args$resid.minval)] <- misty::chr.color(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L][lower.tri(print.resid[-c(1L, (nrow(print.resid) - 1L):nrow(print.resid)) , -1L])][which(abs(x$result$resid[[x$args$invar]][-nrow(x$result$resid[[x$args$invar]]), ][lower.tri(x$result$resid[[x$args$invar]][-nrow(x$result$resid[[x$args$invar]]), ])]) >= x$args$resid.minval)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
        ...
        # Note
        if (isTRUE(x$args$resid.minval < 1L)) { cat(paste0("\n  Note. Max absolute value for acceptable observed and model-implied discrepancy: ", round(x$args$resid.minval, digits = 2L), "\n")) }
    ...
    if (isTRUE("opdyke" %in% print && !is.null(print.object$opdyke))) {
      ...
      switch(x$args$invar,
             ...
             config = {
               ...
               cat("\n Relative Opdyke Distribution Percentile Matrix: Configural Invariance Model\n\n")
               print.opdyke <- print.object$opdyke$config

               # Threshold invariance
             }, thres = {

               cat("\n  Relative Opdyke Distribution Percentile Matrix: Threshold Invariance Model\n\n")
               print.opdyke <- print.object$opdyke$thres

               # Metric invariance
             }, metric = {

               cat("\n  Relative Opdyke Distribution Percentile Matrix: Metric Invariance Model\n\n")
               print.opdyke <- print.object$opdyke$metric

               # Scalar invariance
             }, scalar = {

               cat("\n  Relative Opdyke Distribution Percentile Matrix: Scalar Invariance Model\n\n")
               print.opdyke <- print.object$opdyke$scalar

               # Strict invariance
             }, strict = {

               cat("\n  Relative Opdyke Distribution Percentile Matrix: Strict Invariance Model\n\n")
               print.opdyke <- print.object$opdyke$strict
      ...
      ### Relative Opdyke Distribution Percentile Matrix Available ####
        ...
        if (isTRUE(!x$args$long)) {
          ...
          # Round
          print.opdyke <- lapply(print.opdyke, function(y) ifelse(!is.na(y), formatC(y, digits = p.digits, format = "f", zero.print = ifelse(p.digits > 0L, paste0("0.", paste(rep(0L, times = p.digits), collapse = "")), "0")), NA))

          # Lower triangular
          for (i in seq_along(print.opdyke)) {

            print.opdyke[[i]][upper.tri(print.opdyke[[i]])] <- ""
            diag(print.opdyke[[i]]) <- ""

          }

          # Column names
          print.opdyke <- lapply(print.opdyke, function(y) rbind(colnames(y), y))
          print.opdyke <- lapply(print.opdyke, function(y) cbind(rownames(y), y))

          # Add blank spaces
          for (i in seq_along(print.opdyke)) { print.opdyke[[i]][, 1L] <- paste("   ", print.opdyke[[i]][, 1L]) }

          # Justify
          for (i in seq_along(print.opdyke)) {

            print.opdyke[[i]][, 1L] <- format(print.opdyke[[i]][, 1L], justify = "left")
            print.opdyke[[i]][, -1L] <- apply(print.opdyke[[i]][, -1L], 2L, function(y) format(paste0(ifelse(any((nchar(colnames(print.opdyke[[i]])[-1L]) - p.digits) < 3L), " ", ""), y), justify = "right"))


          }

          # Replace NA
          for (i in seq_along(print.opdyke)) { print.opdyke[[i]][-1L, -1L] <- gsub("NA", "  ", print.opdyke[[i]][-1L, -1L]) }
          ...
          if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
            ...
            for (i in seq_along(print.opdyke)) {
              ...
              print.opdyke[[i]][-1L, -1L][lower.tri(print.opdyke[[i]][-1L, -1L])][print.opdyke[[i]][-1L, -1L][lower.tri(print.opdyke[[i]][-1L, -1L])] |> (\(p) as.numeric(ifelse(misty::chr.trim(p) != "NA", p, NA)))() |> (\(q) which(q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L]))()] <- print.opdyke[[i]][-1L, -1L][lower.tri(print.opdyke[[i]][-1L, -1L])][print.opdyke[[i]][-1L, -1L][lower.tri(print.opdyke[[i]][-1L, -1L])] |> (\(p) as.numeric(ifelse(misty::chr.trim(p) != "NA", p, NA)))() |> (\(q) which(q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L]))()] |>
                (\(q) na.omit(q[ifelse(!is.na(q), q < x$args$opdyke.minmax[1L] | q > x$args$opdyke.minmax[2L], NA)]))() |>
                (\(r) ifelse(!is.na(r), misty::chr.color(r, color = x$args$color, bg = "none", style = x$args$style, check = FALSE), NA))()
          ...
          for (i in seq_along(print.opdyke)) {
            ...
            if (isTRUE(!x$args$long)) { cat("   Group:", names(print.opdyke)[i], "\n") }

            .write.table(print.opdyke[[i]], left = 3L, right = 5L, horiz = horiz)

            if (isTRUE(i != length(print.opdyke))) { cat("\n") }
  ...
  # Coefficient Omega, item.omega() --------------------------------------------
    ...
    if (isTRUE("omega" %in% print)) {
      ...
      print.object$omega$n.items <- format(print.object$omega$n.items, justify = "right")

      #—————————————————————————————————————— #
      ### Round ####

      print.object$omega[, c("omega", "low", "upp")] <- .round(print.object$omega[, c("omega", "low", "upp")], digits = r.digits)

      #—————————————————————————————————————— #
      ### Variable Names ####

      print.object$omega <- rbind(c("  n", "nNA", "nItems", "Omega", "Low", "Upp"), print.object$omega)
    ...
    if (isTRUE("item" %in% print)) {
      ...
      print.object$itemstat[, c("m", "sd", "min", "max")] <- .round(print.object$itemstat[, c("m", "sd", "min", "max")], digits = digits)

      print.object$itemstat[, c("std.ld", "omega", "d.omega")] <- .round(print.object$itemstat[, c("std.ld", "omega", "d.omega")], digits = r.digits)

      #—————————————————————————————————————— #
      ### Variable Names ####

      print.object$itemstat <- rbind(c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max", "Std.Ld", "Omega", "dOmega"), print.object$itemstat)
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")))) {
        ...
        print.object$itemstat[1L, "omega"] <- sub("Omega", "\U03C9", print.object$itemstat[1, "omega"])
        print.object$itemstat[1L, "d.omega"] <- sub("dOmega", "\U0394\U03C9", print.object$itemstat[1, "d.omega"])
      ...
      switch(x$args$type, "omega" = {
        ...
        if (any(!is.na(x$result$itemstat$omega))) { cat(" Standardized Factor Loading and Coefficient Omega if Item Deleted\n\n") } else { cat(" Standardized Factor Loadings\n\n") }

      }, "hierarch" = {

        if (any(!is.na(x$result$itemstat$omega))) { cat(" Standardized Factor Loading and Hierarchical Omega if Item Deleted\n\n") } else { cat(" Standardized Factor Loadings\n\n") }

      }, "categ" = {

        if (any(!is.na(x$result$itemstat$omega))) { cat(" Standardized Factor Loading and Categorical Omega if Item Deleted\n\n") } else { cat(" Standardized Factor Loadings\n\n") }

      })

      # Print output
      if (any(!is.na(x$result$itemstat$omega))) { .write.table(print.object$itemstat, left = 1L, right = 3L, horiz = horiz) } else { .write.table(print.object$itemstat[, colnames(print.object$itemstat) %in% c("omega", "d.omega")], left = 1L, right = 3L, horiz = horiz) }
  ...
  # Item Analysis, item.stats() ------------------------------------------------
    ...
    if (isTRUE(any(print.object$pNA != 0L))) { print.object$pNA <- paste0(.round(print.object$pNA, digits = digits), "%") }

    if (isTRUE("p" %in% names(print.object))) { print.object$p <- .round(print.object$p, digits = digits) }
    if (isTRUE("m" %in% names(print.object))) { print.object$m <- .round(print.object$m, digits = digits) }
    if (isTRUE("med" %in% names(print.object))) { print.object$med <- .round(print.object$med, digits = digits) }
    if (isTRUE("min" %in% names(print.object))) { print.object$min <- .round(print.object$min, digits = digits) }
    if (isTRUE("max" %in% names(print.object))) { print.object$max <- .round(print.object$max, digits = digits) }

    print.object$sd <- .round(print.object$sd, digits = digits)

    print.object[, c("r", "low", "upp", "alpha", "d.alpha")] <- .round(print.object[, c("r", "low", "upp", "alpha", "d.alpha")], digits = r.digits)

    #—————————————————————————————————————— #
    ### Variable Name ####

    print.object <- switch(x$dtype,
                           "dicho" = { rbind(c("Item", "n", "nNA", "%NA", "n0", "n1", "P", "SD", "r", "Low", "Upp", "Alpha", "dAlpha"), print.object) },
                           "poly"  = { rbind(c("Item", "n", "nNA", "%NA", colnames(print.object)[5L:(which(colnames(print.object) == "m") - 1L)], "M", "Med", "SD", "Min", "Max", "r", "Low", "Upp", "Alpha", "dAlpha"), print.object) },
                           "cont"  = { rbind(c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max", "r", "Low", "Upp", "Alpha", "dAlpha"), print.object) })

    # R Markdown not in progress
    if (isTRUE(is.null(getOption("knitr.in.progress")))) {

      print.object[1L, "alpha"] <- sub("Alpha", "\U03B1", print.object[1, "alpha"])
      print.object[1L, "d.alpha"] <- sub("dAlpha", "\U0394\U03B1", print.object[1, "d.alpha"])

    }

    # Replace NA
    print.object[, -c(1L:4)] <- apply(print.object[, -c(1L:4)], 2L, function(y) gsub("NA", "  ", y))
    ...
    switch(x$dtype, "dicho" = {

      cat(" Item Statistics for Dichotomous Data\n\n", paste0(ifelse(x$args$correct, " Corrected", ""), " Item-Total ", ifelse(x$args$method == "pbiser", "Point-Biserial", "Biserial"), " Correlation and Alpha if Item Deleted\n\n"))

    }, "poly" = {

      cat(" Item Statistics for Polytomous Data\n\n", paste0(ifelse(x$args$correct, " Corrected", ""), " Item-Total Polyserial Correlation and Ordinal Alpha if Item Deleted\n\n"))

    }, "cont" = {

      cat(" Item Statistics for Continuous Data\n\n", paste0(ifelse(x$args$correct, " Corrected", ""), " Item-Total Product-Moment Correlation and Alpha if Item Deleted\n\n"))
  ...
  # Model Comparison, modcomp() ------------------------------------------------
    ...
    if (isTRUE(nrow(print.object) == 1L)) { if (isTRUE(is.null(x$args$print.ic))) { cat(" Model Fit Indices\n\n") } else if (isTRUE(!is.null(x$args$print.ic) && is.null(x$args$print.fit))) { cat(" Information Criteria\n\n") } else if (isTRUE(is.null(x$args$print.ic) && is.null(x$args$print.fit))) { cat(" Model Fit Indices and Information Criteria\n\n") } } else { cat(" Model Comparison\n\n") }

    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## Format ####

    # Round fit indices
    if (isTRUE(any(colnames(print.object) %in% c("deviance", "chisq", "d.chisq", "F", "cfi", "tli", "rmsea", "gfi", "srmr", "srmrw", "srmrb")))) { intersect(colnames(print.object), c("deviance", "chisq", "d.chisq", "F", "cfi", "tli", "rmsea", "gfi", "srmr", "srmrw", "srmrb")) |>
        (\(p) names(which(sapply(print.object[, p, drop = FALSE], function(y) !all(is.na(y))))))() |> (\(q) print.object[, q] <<- lapply(print.object[, q, drop = FALSE], function(y) sapply(y, function(z) formatC(z, digits = x$args$fit.digits, format = "f", zero.print = ifelse(x$args$fit.digits > 0L, paste0("0.", paste(rep(0L, times = x$args$fit.digits), collapse = "")), "0")))))() }

    # Round information criteria
    if (isTRUE(!is.null(x$args$print.ic))) { intersect(colnames(print.object), c("aic", "caic", "bic", "sabic", "aicc", "hqc", "hbic", "spbic", "ibic", "sic", "icomp")) |>
        (\(p) names(which(sapply(print.object[, p, drop = FALSE], function(y) !all(is.na(y))))))() |> (\(q) print.object[, q] <<- lapply(print.object[, q, drop = FALSE], function(y) sapply(y, function(z) formatC(z, digits = x$args$ic.digits, format = "f", zero.print = ifelse(x$args$ic.digits > 0L, paste0("0.", paste(rep(0L, times = x$args$ic.digits), collapse = "")), "0")))))() }

    # Round p-value
    if (isTRUE("p" %in% colnames(print.object))) { print.object$p <- formatC(print.object$p, digits = p.digits, format = "f", zero.print = ifelse(p.digits > 0L, paste0("0.", paste(rep(0L, times = p.digits), collapse = "")), "0")) }

    # Format
    print.object <- rbind(misty::rec(colnames(print.object), spec = "'model' = 'Model'; 'npar' = '#Param'; 'deviance' = 'Deviance'; 'chisq' = 'Chisq'; 'cfi' = 'CFI'; 'tli' = 'TLI'; 'rmsea' = 'RMSEA'; 'gfi' = 'GFI'; 'srmr' = 'SRMR'; 'srmrw' = 'SRMRw'; 'srmrb' = 'SRMRb'; 'aic' = 'AIC'; 'caic' = 'CAIC'; 'bic' = 'BIC'; 'sabic' = 'SABIC'; 'aicc' = 'AICc'; 'hqc' = 'HQC'; 'hbic' = 'HBIC'; 'spbic' = 'SPBIC'; 'ibic' = 'IBIC'; 'sic' = 'SIC'; 'icomp' = 'ICOMP'; 'd.chisq' = 'dChisq'; 'd.df' = 'ddf'"), print.object)

    # Greek letters
    if (isTRUE(is.null(getOption("knitr.in.progress")))) {

      if (isTRUE("chisq" %in% colnames(print.object))) { print.object[1L, "chisq"] <- "\U03C7\U00B2" }
      if (isTRUE("d.chisq" %in% colnames(print.object))) { print.object[1L, "d.chisq"] <- "\U2206\U03C7\U00B2" }
      if (isTRUE("d.df" %in% colnames(print.object))) { print.object[1L, "d.df"] <- paste0("\U2206", "df") }
  ...
  #  Multilevel Coefficient Alpha, multilevel.alpha() --------------------------
    ...
    if (isTRUE("alpha" %in% print)) {
      ...
      # Extract result table
      print.alpha <- print.object$alpha

      # Round
      print.alpha[, setdiff(colnames(print.alpha), c("type", "n.items"))] <- .round(print.alpha[, setdiff(colnames(print.alpha), c("type", "n.items"))], digits = r.digits)
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")))) {
        ...
        print.alpha[, "type"] <- sub("alpha.w", "\U03B1\U02B7", print.alpha[, "type"])
        print.alpha[, "type"] <- sub("alpha.b", "\U03B1\U1D47", print.alpha[, "type"])
        print.alpha[, "type"] <- sub("alpha.2l", "\U03B1\U00B2\U02E1", print.alpha[, "type"])

      }

      # Include spaces
      print.alpha[, "type"] <- paste0("   ", print.alpha[, "type"])

      # Include labels
      print.alpha <- if (isTRUE(x$args$se != "none")) { rbind(c("  Type", "nItems", "Alpha", "Low", "Upp"), print.alpha) } else { rbind(c("   Type", "nItems", "Alph"), print.alpha) }

      # Justify left
      print.alpha[, "type"] <- format(print.alpha[, 1L], justify = "left")

      # Justify right
      print.alpha[, -1L] <- format(print.alpha[, -1L], justify = "right")

      # Print
      if (isTRUE(x$args$se != "none")) {

        cat(paste0(" Multilevel Coefficient Alpha with ", x$args$conf.level*100L, "% Confidence Interval\n\n"))

      } else {

        cat(" Multilevel Coefficient Alpha\n\n")

      }

      .write.table(print.alpha, left = 1L, right = 3L, horiz = horiz)
    ...
    if (isTRUE("item" %in% print)) {
      ...
      print.item[, print.round] <- .round(print.item[, print.round], digits = digits)
      print.item[, c("ICC", "wstd.ld", "bstd.ld")] <- .round(print.item[, c("ICC", "wstd.ld", "bstd.ld")], digits = r.digits)
      ...
      # Column names
      print.item <- rbind(c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max",  "Skew", "Kurt", "ICC(1)", "WStd.ld", "BStd.ld"), print.item)
      ...
      # Add blank space
      print.item[, "item"] <- c(paste0("  ", print.item[1L, "item"], collapse = ""), paste0("   ", print.item[-1L, "item"]))
      print.item[, "item"] <- format(c(print.item[1L, "item"], misty::chr.trim(print.item[-1L, "item"], side = "right")), justify = "left")
      ...
      cat(" Standardized Factor Loadings at the Within and Between Level\n\n")

      .write.table(print.item, left = 1L, right = 3L, horiz = horiz)
  ...
  # Multilevel Confirmatory Factor Analysis, multilevel.cfa() ------------------
    ...
    if (isTRUE("coverage" %in% print && !is.null(print.object$coverage))) {
      ...
      print.coverage <- .round(print.coverage, digits = p.digits - 1L)
      ...
      print.coverage <- apply(print.coverage, 2L, function(y) gsub("NA", "  ", y))
    ...
    if (isTRUE("fit" %in% print && !is.null(print.object$fit))) {
      ...
      if (isTRUE(!x$args$ls.fit)) {
        ...
        # Within/Between
        print.fit[which(misty::chr.trim(print.fit[, 1L]) %in% c("Within", "Between")), 1L] <- paste(" ", unlist(print.fit[which(misty::chr.trim(print.fit[, 1L]) %in% c("Within", "Between")), 1L]))

        # RMSEA and GFI 95% CI - p-values
        print.fit[which(misty::chr.trim(print.fit[, 1L]) %in% c("90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05")), 1L] <- paste("", unlist(print.fit[which(misty::chr.trim(print.fit[, 1]) %in% c("90 Percent CI - lower", "90 Percent CI - upper", "P-value RMSEA <= 0.05")), 1L]))
      ...
      ### Simultaneous and Level-Specific Model Fit Information ####
        ...
        if (isTRUE(ncol(print.fit) == 2L)) {
          ...
          # Fit indices
          print.fit[-c(which(print.fit[, 1L] %in% c("H0 Value, Specified Model", "Scaling Correction Factor", "H1 Value, Unrestricted Model", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC", "Test statistic", "Degrees of freedom", "P-value", "P-value RMSEA <= 0.05")), 12L:13L) , -1L] <- sapply(print.fit[-c(which(print.fit[, 1L] %in% c("H0 Value, Specified Model", "Scaling Correction Factor", "H1 Value, Unrestricted Model", "Scaling Correction Factor", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC", "Test statistic", "Degrees of freedom", "P-value", "P-value RMSEA <= 0.05")), 12L:13L), -1L], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), NA))

          # Degrees of freedom
          print.fit[14L:16L, -1] <- sapply(print.fit[14L:16L, -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = 0L, format = "f"), NA))

          # Loglikelihood, information criteria, and test statistic
          print.fit[c(2L:3L, 6L:8L, 11L:13L), -1] <- sapply(print.fit[c(2L:3L, 6L:8L, 11L:13L), -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits - 1L, format = "f"), NA))
        ...
        # Robust Maximum Likelihood
          ...
          # Fit indices
          print.fit[-c(which(print.fit[, 1L] %in% c("H0 Value, Specified Model", "Scaling Correction Factor", "H1 Value, Unrestricted Model", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC", "Test statistic", "Degrees of freedom", "P-value", "P-value RMSEA <= 0.05")), 14L:15L) , -1L] <- sapply(print.fit[-c(which(print.fit[, 1L] %in% c("H0 Value, Specified Model", "Scaling Correction Factor", "H1 Value, Unrestricted Model", "Scaling Correction Factor", "Akaike (AIC)", "Bayesian (BIC)", "Sample-Size Adjusted BIC", "Test statistic", "Degrees of freedom", "P-value", "P-value RMSEA <= 0.05")), 14L:15L), -1L], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0")), NA))

          # Scaling correction factor
          print.fit[22L, -1] <- ifelse(!is.na(print.fit[22L, -1]), formatC(as.numeric(print.fit[22L, -1]), digits = digits, format = "f"), NA)

          # Degrees of freedom
          print.fit[16L:18L, -1] <- sapply(print.fit[16L:18L, -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = 0L, format = "f"), NA))

          # Loglikelihood, information criteria, and test statistic
          print.fit[c(2L:5L, 8L:10L, 13L:15L), -1] <- sapply(print.fit[c(2L:5L, 8L:10L, 13L:15L), -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits - 1L, format = "f"), NA))
        ...
        print.fit[!misty::chr.trim(print.fit[, 1L]) %in% c("", "Loglikelihood", "Information Criteria", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices"), 1L] <- paste("", unlist(print.fit[!misty::chr.trim(print.fit[, 1L]) %in% c("", "Loglikelihood", "Information Criteria", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices"), 1L]))

        # Within/Between
        print.fit[misty::chr.trim(print.fit[, 1L]) %in% c("Within", "Between"), 1L] <- paste("", unlist(print.fit[misty::chr.trim(print.fit[, 1L]) %in% c("Within", "Between"), 1L]))

        # RMSEA 95% CI - p-values
        print.fit[misty::chr.trim(print.fit[, 1L]) %in% c("90 Percent CI - Lower", "90 Percent CI - Upper", "P-value RMSEA <= 0.05"), 1L] <- paste("", unlist(print.fit[misty::chr.trim(print.fit[, 1L]) %in% c("90 Percent CI - Lower", "90 Percent CI - Upper", "P-value RMSEA <= 0.05"), 1L]))

        # RMSEA and GFI Within/Between
        if (isTRUE(ncol(print.fit) == 2L)) {

          # Maximum Likelihood
          print.fit[c(39L:40L, 42L:43L, 45L:46L, 57L:58L, 60L:61L), 1L] <- paste("", unlist(print.fit[c(39L:40L, 42L:43L, 45L:46L, 57L:58L, 60L:61L), 1L]))

        } else {

          # Robust maximum Likelihood
          print.fit[c(44L:45L, 47L:48L, 50L:51L, 62L:63L, 65L:66L), 1L] <- paste("", unlist(print.fit[c(44L:45L, 47L:48L, 50L:51L, 62L:63L, 65L:66L), 1L]))
    ...
    if (isTRUE("est" %in% print && !is.null(print.object$param))) {
      ...
      # digits
      print.param.w[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs", "pvalue"))] <- .round(print.param.w[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs", "pvalue"))], digits = digits)

      # p.digits
      if (isTRUE("pvalue" %in% colnames(print.param.w))) { print.param.w[, "pvalue"] <- .round(as.numeric(print.param.w[, "pvalue"]), digits = p.digits) }

      #···················
      #### Convert NA into "" ####

      print.param.w[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs"))] <- as.data.frame(sapply(print.param.w[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs"))], function(y) gsub("NA", "  ", y)))

      #···················
      #### Add Blank Spaces ####

      print.param.w[grep("~", print.param.w$rhs), "rhs"] <- paste("   ", print.param.w[grep("~", print.param.w$rhs), "rhs"])
      print.param.w[-grep("~", print.param.w$rhs), "rhs"] <- paste("      ", print.param.w[-grep("~", print.param.w$rhs), "rhs"])
      ...
      if (isTRUE(x$args$se  != "none")) {
        ...
        print.param.w <- rbind(c("", "", "", "", "Estimate", "StdYX"), print.param.w)
      ...
      # Columns
      columns <- intersect(colnames(print.param.w), c("rhs", "est", "se", "z", "pvalue", "stdyx"))
      ...
      if (isTRUE(any(print.param.w$param %in% "latent variable"))) {
        ...
        # Heading
        write.table(print.param.w[1L, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)

        # Horizontal line
        if (isTRUE(horiz)) { cat(paste(rep(" ", times = 2), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param.w[1L, columns])) + length(print.param.w[1L, columns]) - 4L), collapse = ""), "\n") }
      ...
      ##### Shared construct ####
        ...
        # Horizontal line
        if (isTRUE(horiz)) { cat(paste(rep(" ", times = 2), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param.w[1L, columns])) + length(print.param.w[1L, columns]) - 4), collapse = ""), "\n") }
      ...
      if (isTRUE(any(print.param.w$param %in% "latent variable covariance"))) {
        ...
        for (i in unique(print.lv.cov$lhs)) { write.table(print.lv.cov[print.lv.cov$lhs == i, columns], quote = FALSE, row.names = FALSE, col.names = FALSE) }
      ...
      if (isTRUE(any(print.param.w$param %in% "residual covariance"))) {
        ...
        for (i in unique(print.res.cov$lhs)) { write.table(print.res.cov[print.res.cov$lhs == i, columns], quote = FALSE, row.names = FALSE, col.names = FALSE) }
      ...
      if (isTRUE(any(print.param.w$param %in% "latent variance"))) {
        ...
        if (isTRUE("z" %in% colnames(print.var))) { print.var[grep("NA", print.var[, "z"]), setdiff(columns, "rhs")] <- "" }
      ...
      # digits
      print.param.b[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs", "pvalue"))] <- .round(print.param.b[, setdiff(colnames(print.param.w), c("param", "lhs", "op", "rhs", "pvalue"))], digits = digits)

      # p.digits
      if (isTRUE("pvalue" %in% colnames(print.param.w))) { print.param.b[, "pvalue"] <- .round(as.numeric(print.param.b[, "pvalue"]), digits = p.digits) }

      #···················
      #### Convert NA into "" ####

      print.param.b[, setdiff(colnames(print.param.b), c("param", "lhs", "op", "rhs"))] <- as.data.frame(sapply(print.param.b[, setdiff(colnames(print.param.b), c("param", "lhs", "op", "rhs"))], function(y) gsub("NA", "  ", y)))

      #...................
      #### Add blank spaces ####

      print.param.b[grep("~", print.param.b$rhs), "rhs"] <- paste("   ", print.param.b[grep("~", print.param.b$rhs), "rhs"])
      print.param.b[-grep("~", print.param.b$rhs), "rhs"] <- paste("      ", print.param.b[-grep("~", print.param.b$rhs), "rhs"])
      ...
      if (isTRUE(x$args$se  != "none")) {
        ...
        print.param.b <- rbind(c("", "", "", "", "Estimate", "StdYX"), print.param.b)
      ...
      if (isTRUE(any(print.param.b$param %in% "latent variable"))) {
        ...
        # Heading
        write.table(print.param.b[1L, columns], quote = FALSE, row.names = FALSE, col.names = FALSE)

        # Horizontal line
        if (isTRUE(horiz)) { cat(paste(rep(" ", times = 2), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param.b[1L, columns])) + length(print.param.b[1L, columns]) - 4), collapse = ""), "\n") }
        ...
        for (i in unique(print.lv$lhs)) {
          ...
          if (isTRUE(x$args$se  != "none")) {
            ...
            if (isTRUE(length(pos.NA) > 0L)) {
              ...
              print.lv[print.lv$lhs == i, ][pos.NA, "est"] <- paste(rep("", times = unique(nchar(print.lv[print.lv$lhs == i, ][pos.NA, "est"])) + 1L), collapse = " ")
              print.lv[print.lv$lhs == i, ][pos.NA, "se"] <- paste(rep("", times = unique(nchar(print.lv[print.lv$lhs == i, ][pos.NA, "se"])) + 1L), collapse = " ")
              print.lv[print.lv$lhs == i, ][pos.NA, "z"] <- paste(rep("", times = unique(nchar(print.lv[print.lv$lhs == i, ][pos.NA, "z"])) + 1L), collapse = " ")
              print.lv[print.lv$lhs == i, ][pos.NA, "pvalue"] <- paste(rep("", times = unique(nchar(print.lv[print.lv$lhs == i, ][pos.NA, "pvalue"])) + 1L), collapse = " ")
              print.lv[print.lv$lhs == i, ][pos.NA, "stdyx"] <- paste(rep("", times = unique(nchar(print.lv[print.lv$lhs == i, ][pos.NA, "stdyx"])) + 1L), collapse = " ")
    ...
    if (isTRUE("modind" %in% print && !is.null(print.object$modind))) {
      ...
      #### Modification Indices Available ####
        ...
        #### Modification indices
          ...
          # Round
          print.modind.w[, -c(1L:3L)] <- .round(print.modind.w[, -c(1L:3L)], digits = digits)
          ...
          if (isTRUE(nrow(print.modind.w.load) > 0L)) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.modind.w[1L, ])) + length(print.modind.w[1L, ]) - 5), collapse = ""), "\n") }
        ...
        #### Modification Indices ####
          ...
          # Round
          print.modind.b[, -c(1L:3L)] <- .round(print.modind.b[, -c(1L:3L)], digits = digits)
          ...
          if (isTRUE(nrow(print.modind.b.load) > 0L)) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.modind.b[1L, ],)) + length(print.modind.b[1L, ]) - 5), collapse = ""), "\n") }
      ...
      #### Score Tests Available ####
        ...
        # Round
        print.score[, c("mi", "lhs.epc", "rhs.epc", "lhs.stdyx", "rhs.stdyx")] <- .round(print.score[, c("mi", "lhs.epc", "rhs.epc", "lhs.stdyx", "rhs.stdyx")], digits = digits)
    ...
    if (isTRUE("resid" %in% print && !is.null(print.object$resid))) {
      ...
      ### Residual Correlation Matrix Available ####
        ...
        # Round
        print.resid <- lapply(print.resid, .round, digits = p.digits)
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && x$args$resid.minval != 1L && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          for (i in seq_along(print.resid)) {
            ...
            print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])), -1L][lower.tri(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])) , -1L])][which(abs(x$result$resid[[i]][-nrow(x$result$resid[[i]]), ][lower.tri(x$result$resid[[i]][-nrow(x$result$resid[[i]]), ])]) >= x$args$resid.minval)] <- misty::chr.color(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])) , -1L][lower.tri(print.resid[[i]][-c(1L, (nrow(print.resid[[i]]) - 1L):nrow(print.resid[[i]])) , -1L])][which(abs(x$result$resid[[i]][-nrow(x$result$resid[[i]]), ][lower.tri(x$result$resid[[i]][-nrow(x$result$resid[[i]]), ])]) > x$args$resid.minval)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
    ...
    if (isTRUE("opdyke" %in% print && !is.null(print.object$opdyke))) {
      ...
      ### Relative Opdyke Distribution Percentile Matrix Available ####
        ...
        # Round
        print.opdyke <- lapply(print.opdyke, .round, digits = p.digits)
        ...
        print.opdyke$within <- apply(print.opdyke$within, 2L, function(y) gsub("NA", "  ", y))
        print.opdyke$between <- apply(print.opdyke$between, 2L, function(y) gsub("NA", "  ", y))
        ...
        if (isTRUE(any(!print.opdyke$within[-1L, -1L] %in% c(" NA", "   ")))) {

          .write.table(print.opdyke$within, left = 2L, right = 4L, horiz = horiz)

        } else {

          cat("   Relative Opdyke distribution percentile matrix at the Within level is not available.")
        ...
        if (isTRUE(any(!misty::chr.trim(print.opdyke$between[-1L, -1L]) %in% c("NA", "")))) {

          .write.table(print.opdyke$between, left = 2L, right = 4L, horiz = horiz)

        } else {

          cat("\n    Relative Opdyke distribution percentile matrix at the Between level is not available.")
  ...
  # Within-Group and Between-Group Correlation Matrix, multilevel.cor() --------
    ...
    if (isTRUE(x$args$split)) {
      ...
      if (isTRUE(x$args$se != "none")) {
        ...
        print.object$with.cor <- .round(print.object$with.cor, digits = digits)
        print.object$with.se <- .round(print.object$with.se, digits = digits)
        print.object$with.stat <- .round(print.object$with.stat, digits = digits)
        print.object$with.p <- .round(print.object$with.p, digits = p.digits)

        # Round and format Between
        print.object$betw.cor <- .round(print.object$betw.cor, digits = digits)
        print.object$betw.se <- .round(print.object$betw.se, digits = digits)
        print.object$betw.stat <- .round(print.object$betw.stat, digits = digits)
        print.object$betw.p <- .round(print.object$betw.p, digits = p.digits)

        diag(print.object$with.p) <- diag(print.object$with.stat) <- diag(print.object$with.se) <- diag(print.object$with.cor) <- ""
        diag(print.object$betw.p) <- diag(print.object$betw.stat) <- diag(print.object$betw.se) <- diag(print.object$betw.cor) <- ""
        ...
        switch(x$args$tri, "lower" = {
          ...
          # Within
          print.object$with.p[upper.tri(print.object$with.p)] <- print.object$with.stat[upper.tri(print.object$with.stat)] <- print.object$with.se[upper.tri(print.object$with.se)] <- print.object$with.cor[upper.tri(print.object$with.cor)] <- ""

          # Between
          print.object$betw.p[upper.tri(print.object$betw.p)] <- print.object$betw.stat[upper.tri(print.object$betw.stat)] <- print.object$betw.se[upper.tri(print.object$betw.se)] <- print.object$betw.cor[upper.tri(print.object$betw.cor)] <- ""

        }, "upper" = {

          # Within
          print.object$with.p[lower.tri(print.object$with.p)] <- print.object$with.stat[lower.tri(print.object$with.stat)] <- print.object$with.se[lower.tri(print.object$with.se)] <- print.object$with.cor[lower.tri(print.object$with.cor)] <- ""

          # Between
          print.object$betw.p[lower.tri(print.object$betw.p)] <- print.object$betw.stat[lower.tri(print.object$betw.stat)] <- print.object$betw.se[lower.tri(print.object$betw.se)] <- print.object$betw.cor[lower.tri(print.object$betw.cor)] <- ""
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          print.object$with.cor[-1, -1L][lower.tri(print.object$with.cor[-1, -1L])][which(x$result$with.p[lower.tri(x$result$with.p)] <= x$args$alpha)] <- misty::chr.color(print.object$with.cor[-1, -1L][lower.tri(print.object$with.cor[-1, -1L])][which(x$result$with.p[lower.tri(x$result$with.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
          print.object$with.cor[-1, -1L][upper.tri(print.object$with.cor[-1, -1L])][which(x$result$with.p[upper.tri(x$result$with.p)] <= x$args$alpha)] <- misty::chr.color(print.object$with.cor[-1, -1L][upper.tri(print.object$with.cor[-1, -1L])][which(x$result$with.p[upper.tri(x$result$with.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)

          print.object$betw.cor[-1, -1L][lower.tri(print.object$betw.cor[-1, -1L])][which(x$result$betw.p[lower.tri(x$result$betw.p)] <= x$args$alpha)] <- misty::chr.color(print.object$betw.cor[-1, -1L][lower.tri(print.object$betw.cor[-1, -1L])][which(x$result$betw.p[lower.tri(x$result$betw.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
          print.object$betw.cor[-1, -1L][upper.tri(print.object$betw.cor[-1, -1L])][which(x$result$betw.p[upper.tri(x$result$betw.p)] <= x$args$alpha)] <- misty::chr.color(print.object$betw.cor[-1, -1L][upper.tri(print.object$betw.cor[-1, -1L])][which(x$result$betw.p[upper.tri(x$result$betw.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
    ...
    ## Combined Results ####
      ...
      if (isTRUE(x$args$se != "none")) {
        ...
        print.object$wb.cor <- .round(print.object$wb.cor, digits = digits)
        print.object$wb.se <- .round(print.object$wb.se, digits = digits)
        print.object$wb.stat <- .round(print.object$wb.stat, digits = digits)
        print.object$wb.p <- .round(print.object$wb.p, digits = p.digits)

        diag(print.object$wb.p) <- diag(print.object$wb.stat) <- diag(print.object$wb.se) <- diag(print.object$wb.cor) <- ""
        ...
        if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
          ...
          print.object$wb.cor[-1L, -1L][lower.tri(print.object$wb.cor[-1L, -1L])][which(x$result$wb.p[lower.tri(x$result$wb.p)] <= x$args$alpha)] <- misty::chr.color(print.object$wb.cor[-1L, -1L][lower.tri(print.object$wb.cor[-1L, -1L])][which(x$result$wb.p[lower.tri(x$result$wb.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
          print.object$wb.cor[-1L, -1L][upper.tri(print.object$wb.cor[-1L, -1L])][which(x$result$wb.p[upper.tri(x$result$wb.p)] <= x$args$alpha)] <- misty::chr.color(print.object$wb.cor[-1L, -1L][upper.tri(print.object$wb.cor[-1L, -1L])][which(x$result$wb.p[upper.tri(x$result$wb.p)] <= x$args$alpha)], color = x$args$color, bg = "none", style = x$args$style, check = FALSE)
    ...
    if (isTRUE("summary" %in% print)) {
      ...
      if (isTRUE(any(c("cor", "se", "stat", "p") %in% print))) { cat("\n") }
    ...
    if (isTRUE(!x$args$split)) {
      ...
      if (isTRUE("cor" %in% print)) {
        ...
        if (isTRUE(!"summary" %in% print)) { cat("\n") }

        .write.table(print.object$wb.cor, left = 2L, right = 4L, horiz = horiz)
    ...
    if (isTRUE(any(c("cor", "se", "stat", "p") %in% print))) {

      # Lower and upper triangular
      if (isTRUE(!x$args$split)) { cat(ifelse(isTRUE(x$args$tri.lower), "\n Note. Lower triangular: Within-Group, Upper triangular: Between-Group", "\n Note. Lower triangular: Between-Group, Upper triangular: Within-Group"), "\n") }
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")) && x$args$color != "default" && ifelse(!is.null(x$args$write), !grepl("\\b.txt\\b", x$args$write), TRUE))) {
        ...
        if (isTRUE(!x$args$split)) {
          ...
          cat(paste0("       Statistically significant coefficients at \U03B1 = ", signif(x$args$alpha, digits = 2L), " are highlighted ", misty::chr.color(x$args$color, color = x$args$color, bg = "none", style = x$args$style, check = FALSE), "\n"))

        } else {

          cat(paste0("\n Note. Statistically significant coefficients at \U03B1 = ", signif(x$args$alpha, digits = 2L), " are highlighted ", misty::chr.color(x$args$color, color = x$args$color, bg = "none", style = x$args$style, check = FALSE), "\n"))
  ...
  #  Multilevel Descriptive Statistics, multilevel.descript() ------------------
    ...
    # Variance and/or SD
    if (isTRUE(!"var" %in% print)) { print.object <- print.object[-grep("Variance", print.object[, 1L]), ] }
    if (isTRUE(!"sd" %in% print)) { print.object <- print.object[-grep("SD", print.object[, 1L]), ] }
  ...
  # Simultaneous and LS Multilevel Model Fit Information, multilevel.fit() -----
    ...
    if (isTRUE("fit" %in% print)) {
      ...
      if (isTRUE(ncol(print.fit) == 2L)) {
        ...
        # Degrees of freedom
        print.fit[which(print.fit[, 1L] == "Degrees of Freedom") |> (\(p) p:(p + 2L))(), -1L] <- sapply(print.fit[which(print.fit[, 1L] == "Degrees of Freedom") |> (\(p) p:(p + 2L))(), -1L], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = 0L, format = "f"), NA))

        # Loglikelihood, information criteria, and test statistic
        print.fit[c(which(print.fit[, 1L] == "Loglikelihood") |> (\(p) (p + 1L):(p + 2L))(), which(print.fit[, 1L] == "Information Criteria") |> (\(p) (p + 1L):(p + 3L))(), which(print.fit[, 1L] == "Chi-Square Test of Model Fit") |> (\(p) (p + 1L):(p + 3L))()), -1] <- sapply(print.fit[c(which(print.fit[, 1L] == "Loglikelihood") |> (\(p) (p + 1L):(p + 2L))(), which(print.fit[, 1L] == "Information Criteria") |> (\(p) (p + 1L):(p + 3L))(), which(print.fit[, 1L] == "Chi-Square Test of Model Fit") |> (\(p) (p + 1L):(p + 3L))()), -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits - 1L, format = "f"), NA))

        # Add labels
        print.fit <- rbind(print.fit[which(print.fit[, 1L] == "Loglikelihood"):(which(print.fit[, 1L] == "Chi-Square Test of Model Fit") - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Chi-Square Test of Model Fit"):(which(print.fit[, 1L] == "Incremental Fit Indices") - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Incremental Fit Indices"):(which(print.fit[, 1L] == "Absolute Fit Indices") - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Absolute Fit Indices"):(which(print.fit[, 1L] == "Coefficient of Determination") - 1L), ], c("", "Standard"),
                           print.fit[which(print.fit[, 1L] == "Coefficient of Determination"):nrow(print.fit), ],
      ...
      # Robust Maximum Likelihood
        ...
        # Scaling correction factor
        print.fit[(which(print.fit[, 1L] == "P-value") + 3L), -1L] <- ifelse(!is.na(print.fit[(which(print.fit[, 1L] == "P-value") + 3L), -1]), formatC(as.numeric(print.fit[(which(print.fit[, 1L] == "P-value") + 3L), -1]), digits = digits, format = "f"), NA)

        # Degrees of freedom
        print.fit[which(print.fit[, 1L] == "Degrees of Freedom") |> (\(p) p:(p + 2L))(), -1L] <- sapply(print.fit[which(print.fit[, 1L] == "Degrees of Freedom") |> (\(p) p:(p + 2L))(), -1L], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = 0L, format = "f"), NA))

        # Loglikelihood, information criteria, and test statistic
        print.fit[c(which(print.fit[, 1L] == "Loglikelihood") |> (\(p) (p + 1L):(p + 4L))(), which(print.fit[, 1L] == "Information Criteria") |> (\(p) (p + 1L):(p + 3L))(), which(print.fit[, 1L] == "Chi-Square Test of Model Fit") |> (\(p) (p + 1L):(p + 3L))()), -1] <- sapply(print.fit[c(which(print.fit[, 1L] == "Loglikelihood") |> (\(p) (p + 1L):(p + 4L))(), which(print.fit[, 1L] == "Information Criteria") |> (\(p) (p + 1L):(p + 3L))(), which(print.fit[, 1L] == "Chi-Square Test of Model Fit") |> (\(p) (p + 1L):(p + 3L))()), -1], function(y) ifelse(!is.na(y), formatC(as.numeric(y), digits = digits - 1L, format = "f"), NA))

        # Add labels
        print.fit <- rbind(print.fit[which(print.fit[, 1L] == "Loglikelihood"):(which(print.fit[, 1L] == "Chi-Square Test of Model Fit") - 1L), ], c("", "Standard", "Scaled", "Robust"),
                           ...
                           print.fit[which(print.fit[, 1L] == "Absolute Fit Indices"):(which(print.fit[, 1L] == "Coefficient of Determination") - 1L), ], c("", "Standard", "", "Robust"),
                           print.fit[which(print.fit[, 1L] == "Coefficient of Determination"):nrow(print.fit), ],
                           make.row.names = FALSE)
      ...
      print.fit[!misty::chr.trim(print.fit[, 1L]) %in% c("", "Loglikelihood", "Information Criteria", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination"), 1L] <- paste("", unlist(print.fit[!misty::chr.trim(print.fit[, 1L]) %in% c("", "Loglikelihood", "Information Criteria", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination"), 1L]))
      ...
      # RMSEA and GFI Within/Between
      c(which(misty::chr.trim(print.fit[, 1L]) == "90 Percent CI - Lower") |> (\(p) as.vector(sapply(p, function(y) (y + 1L):(y + 2))))(), which(misty::chr.trim(print.fit[, 1L]) == "90 Percent CI - Upper") |> (\(p) as.vector(sapply(p, function(y) (y + 1L):(y + 2))))(), which(misty::chr.trim(print.fit[, 1L]) == "P-value RMSEA <= 0.05") |> (\(p) as.vector(sapply(p, function(y) (y + 1L):(y + 2))))()) |> (\(q) print.fit[q, 1L] <<- paste("", unlist(print.fit[q, 1L])))()
  ...
  # R-Squared Measures for Multilevel and Linear Mixed Effects Models, multilevel.r2()
    ...
    #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## Integrative Framework of R-Squared Measures ####
    ...
    if (isTRUE("NS" %in% print)) {
      ...
      if (isTRUE(any(c("RB", "SB") %in% print))) cat("\n")
    ...
    if (isTRUE("RS" %in% print)) {
      ...
      if (isTRUE(any(c("RB", "SB", "NS") %in% print))) cat("\n")
  ...
  # Cross-Level Measurement Invariance, multilevel.invar() ---------------------
    ...
    if (isTRUE("fit" %in% print)) {
      ...
      # Fit indices
      c(which(print.fit.stand[, 1L] == "Test statistic"):(which(print.fit.stand[, 1L] == "P-value RMSEA <= 0.05") - 1L), which(print.fit.stand[, 1L] == "SRMR"):(which(print.fit.stand[, 1L] == "Information Criteria") - 2L)) |> (\(p) print.fit.stand[p, -1L] <<- .round(print.fit.stand[p, -1L], digits = digits))()
      if (isTRUE(!is.null(print.fit.scaled))) { c(which(print.fit.scaled[, 1L] == "Test statistic"):(which(print.fit.scaled[, 1L] == "P-value RMSEA <= 0.05") - 1L), which(print.fit.scaled[, 1L] == "SRMR"):(which(print.fit.scaled[, 1L] == "Information Criteria") - 2L)) |> (\(p) print.fit.scaled[p, -1L] <<- .round(print.fit.scaled[p, -1L], digits = digits))() }
      if (isTRUE(!is.null(print.fit.robust))) { c(which(print.fit.robust[, 1L] == "Test statistic"):(which(print.fit.robust[, 1L] == "P-value RMSEA <= 0.05") - 1L), which(print.fit.robust[, 1L] == "SRMR"):(which(print.fit.robust[, 1L] == "Information Criteria") - 2L)) |> (\(p) print.fit.robust[p, -1L] <<- .round(print.fit.robust[p, -1L], digits = digits))() }

      # Information criteria
      which(print.fit.stand[, 1L] == "Akaike (AIC)"):nrow(print.fit.stand) |> (\(p) print.fit.stand[p, -1L] <<- sapply(print.fit.stand[p, -1L], function(y) .round(y, digits = digits - 1L)))()
      if (isTRUE(!is.null(print.fit.scaled))) { which(print.fit.scaled[, 1L] == "Akaike (AIC)"):nrow(print.fit.scaled) |> (\(p) print.fit.scaled[p, -1L] <<- sapply(print.fit.scaled[p, -1L], function(y) .round(y, digits = digits - 1L)))()  }
      if (isTRUE(!is.null(print.fit.robust))) { which(print.fit.robust[, 1L] == "Akaike (AIC)"):nrow(print.fit.robust) |> (\(p) print.fit.robust[p, -1L] <<- sapply(print.fit.robust[p, -1L], function(y) .round(y, digits = digits - 1L)))()  }

      # p.digits
      which(print.fit.stand[, 1L] %in% c("P-value", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.stand[p, -1L] <<- sapply(print.fit.stand[p, -1L], function(y) .round(y, digits = p.digits)))()
      if (isTRUE(!is.null(print.fit.scaled))) { which(print.fit.scaled[, 1L] %in% c("P-value", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.scaled[p, -1L] <<- sapply(print.fit.scaled[p, -1L], function(y) .round(y, digits = p.digits)))()  }
      if (isTRUE(!is.null(print.fit.robust))) { which(print.fit.robust[, 1L] %in% c("P-value", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.robust[p, -1L] <<- sapply(print.fit.robust[p, -1L], function(y) .round(y, digits = p.digits)))()  }

      #—————————————————————————————————————— #
      ### Add Labels ####

      print.fit.stand <- rbind(switch(x$args$invar, config = { c("", "Config") }, metric = { c("", "Config", "Metric", "DMetric") }, scalar = { c("", "Config", "Metric", "Scalar", "DMetric", "DScalar") }), print.fit.stand, make.row.names = FALSE)
      if (isTRUE(!is.null(print.fit.scaled))) { print.fit.scaled <- rbind(switch(x$args$invar, config = { c("", "Config") }, metric = { c("", "Config", "Metric", "DMetric") }, scalar = { c("", "Config", "Metric", "Scalar", "DMetric", "DScalar") }), print.fit.scaled, make.row.names = FALSE) }
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust <- rbind(switch(x$args$invar, config = { c("", "Config") }, metric = { c("", "Config", "Metric", "DMetric") }, scalar = { c("", "Config", "Metric", "Scalar", "DMetric", "DScalar") }), print.fit.robust, make.row.names = FALSE) }

      #—————————————————————————————————————— #
      ### Replace NA with "" ####

      print.fit.stand <- unname(misty::na.as(print.fit.stand, na = "", check = FALSE))
      if (isTRUE(!is.null(print.fit.scaled))) { print.fit.scaled <- unname(misty::na.as(print.fit.scaled, na = "", check = FALSE)) }
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust <- unname(misty::na.as(print.fit.robust, na = "", check = FALSE)) }
      ...
      which(!misty::chr.trim(print.fit.stand[, 1L]) %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria")) |> (\(p) print.fit.stand[p, 1L] <<- paste("", unlist(print.fit.stand[p, 1L])))()
      if (isTRUE(!is.null(print.fit.scaled))) { which(!misty::chr.trim(print.fit.scaled[, 1L]) %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria")) |> (\(p) print.fit.scaled[p, 1L] <<- paste("", unlist(print.fit.scaled[p, 1L])))() }
      if (isTRUE(!is.null(print.fit.robust))) { which(!misty::chr.trim(print.fit.robust[, 1L]) %in% c("", "Chi-Square Test of Model Fit", "Incremental Fit Indices", "Absolute Fit Indices", "Coefficient of Determination", "Information Criteria")) |> (\(p) print.fit.robust[p, 1L] <<- paste("", unlist(print.fit.robust[p, 1L])))() }

      # Within/Between
      which(misty::chr.trim(print.fit.stand[, 1L]) == "Within") |> (\(p) p:(p + 1L))() |> (\(p) print.fit.stand[p, 1L] <<- paste("", unlist(print.fit.stand[p, 1L])))()
      if (isTRUE(!is.null(print.fit.scaled))) { which(misty::chr.trim(print.fit.scaled[, 1L]) == "Within") |> (\(p) p:(p + 1L))() |> (\(p) print.fit.scaled[p, 1L] <<- paste("", unlist(print.fit.scaled[p, 1L])))() }
      if (isTRUE(!is.null(print.fit.robust))) { which(misty::chr.trim(print.fit.robust[, 1L]) == "Within") |> (\(p) p:(p + 1L))() |> (\(p) print.fit.robust[p, 1L] <<- paste("", unlist(print.fit.robust[p, 1L])))() }

      # RMSEA 95% CI - p-values
      which(misty::chr.trim(print.fit.stand[, 1L]) %in% c("90 Percent CI - upper", "90 Percent CI - lower", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.stand[p, 1L] <<- paste("", unlist(print.fit.stand[p, 1L])))()
      if (isTRUE(!is.null(print.fit.scaled))) { which(misty::chr.trim(print.fit.scaled[, 1L]) %in% c("90 Percent CI - upper", "90 Percent CI - lower", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.scaled[p, 1L] <<- paste("", unlist(print.fit.scaled[p, 1L])))() }
      if (isTRUE(!is.null(print.fit.robust))) { which(misty::chr.trim(print.fit.robust[, 1L]) %in% c("90 Percent CI - upper", "90 Percent CI - lower", "P-value RMSEA <= 0.05")) |> (\(p) print.fit.robust[p, 1L] <<- paste("", unlist(print.fit.robust[p, 1L])))() }

      #—————————————————————————————————————— #
      ### Replace NA with "" ####

      print.fit.stand <- as.data.frame(apply(print.fit.stand, 2L, function(y) gsub("NA", "  ", y)))
      if (isTRUE(!is.null(print.fit.scaled))) { print.fit.scaled <- as.data.frame(apply(print.fit.scaled, 2L, function(y) gsub("NA", "  ", y))) }
      if (isTRUE(!is.null(print.fit.robust))) { print.fit.robust <- as.data.frame(apply(print.fit.robust, 2L, function(y) gsub("NA", "  ", y))) }
    ...
    if (isTRUE("est" %in% print)) {
      ...
      if (isTRUE(any(print.param.w$param %in% "latent variable"))) {
        ...
        # Horizontal line
        if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param.w[1L, 4L:9L])) + 1), collapse = ""), "\n") }
      ...
      if (isTRUE(any(print.param.b$param %in% "latent variable"))) {
        ...
        # Horizontal line
        if (isTRUE(horiz)) { cat(paste(rep(" ", times = 3), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.param.b[1L, 4L:9L])) + 1), collapse = ""), "\n") }
      ...
      #### Modification available ####
        ...
        ##### Modification indices
          ...
          if (isTRUE(nrow(print.modind.w.load) > 0L)) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 4L), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.modind.w[1L, ]))), collapse = ""), "\n") }
        ...
        #### Modification Indices ####
          ...
          if (isTRUE(nrow(print.modind.b.cov) > 0L)) {
            ...
            # Horizontal line
            if (isTRUE(horiz)) { cat(paste(rep(" ", times = 4L), collapse = ""), paste(rep("\u2500", times = sum(nchar(print.modind.b[1L, ]))), collapse = ""), "\n") }
  ...
  # Multilevel Composite Reliability, multilevel.omega() -----------------------
    ...
    if (isTRUE("omega" %in% print)) {
      ...
      # Round
      print.omega[, setdiff(colnames(print.omega), c("type", "n.items"))] <- .round(print.omega[, setdiff(colnames(print.omega), c("type", "n.items"))], digits = r.digits)
      ...
      if (isTRUE(is.null(getOption("knitr.in.progress")))) {
        ...
        print.omega[, "type"] <- sub("omega.w", "\U03C9\U02B7", print.omega[, "type"])
        print.omega[, "type"] <- sub("omega.b", "\U03C9\U1D47", print.omega[, "type"])
        print.omega[, "type"] <- sub("omega.2l", "\U03C9\U00B2\U02E1", print.omega[, "type"])

      }

      # Include spaces
      print.omega[, "type"] <- paste0("    ", print.omega[, "type"])

      # Include labels
      print.omega <- if (isTRUE(x$args$se != "none")) { rbind(c("   Type", "nItems", "Omega", "Low", "Upp"), print.omega) } else { rbind(c("   Type", "nItems", "Omega"), print.omega) }

      # Justify left
      print.omega[, "type"] <- format(print.omega[, 1L], justify = "left")
    ...
    if (isTRUE("item" %in% print)) {
      ...
      print.item[, print.round] <- .round(print.item[, print.round], digits = digits)
      print.item[, switch(x$args$const, within = c("ICC", "wstd.ld"), shared = c("ICC", "bstd.ld"), config = c("ICC", "wstd.ld", "bstd.ld"))] <- .round(print.item[, switch(x$args$const, within = c("ICC", "wstd.ld"), shared = c("ICC", "bstd.ld"), config = c("ICC", "wstd.ld", "bstd.ld"))], digits = r.digits)
      ...
      print.item <- rbind(switch(x$args$const,
                                 within = c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max",  "Skew", "Kurt", "ICC(1)", "WStd.ld"),
                                 shared = c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max",  "Skew", "Kurt", "ICC(1)", "BStd.ld"),
                                 config = c("Item", "n", "nNA", "%NA", "M", "SD", "Min", "Max",  "Skew", "Kurt", "ICC(1)", "WStd.ld", "BStd.ld")), print.item)
  ...
  # Levene's Test for Homogeneity of Variance, test.levene() -------------------
    ...
    print.object[["test"]][, "SS"] <- formatC(print.object[["test"]][, "SS"], digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0"))
    print.object[["test"]][, "MSS"] <- formatC(print.object[["test"]][, "MSS"], digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0"))
    print.object[["test"]][1L, "F"] <- formatC(print.object[["test"]][1L, "F"], digits = digits, format = "f", zero.print = ifelse(digits > 0L, paste0("0.", paste(rep(0L, times = digits), collapse = "")), "0"))
    print.object[["test"]][1L, "p"] <- formatC(print.object[["test"]][1L, "p"], digits = p.digits, format = "f", zero.print = ifelse(p.digits > 0L, paste0("0.", paste(rep(0L, times = p.digits), collapse = "")), "0"))

    #—————————————————————————————————————— #
    ### Format ####

    print.object[["test"]] <- rbind(c("df", "SS", "MSS", "F", "p"), print.object[["test"]])
    print.object[["test"]] <- cbind(c("", "  Group", "  Residuals", "  Total"), print.object[["test"]])

    print.object[["test"]][3L, c("F", "p")] <- ""

    print.object[["test"]][, -1L] <- apply(print.object[["test"]][, -1L], 2L, format, justify = "right")
    print.object[["test"]][, 1L] <- format(print.object[["test"]][, 1L], justify = "left")

    # Replace NA with ""
    print.object[["test"]][, c("F", "p")] <- apply(print.object[["test"]][, c("F", "p")], 2L, function(y) gsub("NA", "  ", y))
  ...
  # t-test, test.t() -----------------------------------------------------------
    ...
    switch(x$sample, one = {
      ...
      if (!isTRUE(x$args$effsize)) { print.object <- print.object[, -which(colnames(print.object) %in% c("d", "d.low", "d.upp"))] }
    ...
    ## Paired-Sample t-Test ####
      ...
      if (!isTRUE(x$args$descript)) { print.object <- print.object[, -which(colnames(print.object) %in% c("n", "nNA", "m1", "m2", "m.diff", "sd.diff", "se", "m.low", "m.upp"))] }
  ...
  # Welch Test, test.welch() ---------------------------------------------------
    ...
    switch(x$sample, two = {
      ...
      if (isTRUE(x$args$effsize)) {
        ...
        cat(paste("\n  Note. d = Cohen's d with",
                  switch(x$args$alternative, two.sided = "Two-Sided", less = "One-Sided", greater = "One-Sided"),
                  paste0(round(x$args$conf.level * 100L, digits = 2L), "%"), "CI\n"))
    ...
    ## Welch ANOVA ####
      ...
      print.object[["descript"]] <- rbind(c("Group", "n", "nNA", "M", "Low", "Upp", "SD", "Skew", "Kurt"), print.object[["descript"]])
      print.object[["test"]] <- rbind(c("F", "df1", "df2", "p", "es", "os"), print.object[["test"]])
      ...
      if (isTRUE(getOption("knitr.in.progress"))) {
        ...
        print.object[["test"]][1L, ] <- misty::chr.gsub(pattern = c("es", "os"), replacement = c("eta2", "omega2"), x = print.object[["test"]][1L, ])

      # R Markdown not in progress
      } else {

        print.object[["test"]][1L, ] <- misty::chr.gsub(pattern = c("es", "os"), replacement = c("\u03B7\u00B2", "\u03C9\u00B2"), x = print.object[["test"]][1L, ])
  ...
  # z-Test, test.z() -----------------------------------------------------------
    ...
    switch(x$sample, one = {
      ...
      if (!isTRUE(x$args$effsize)) { print.object <- print.object[, -which(colnames(print.object) %in% c("d"))] }
