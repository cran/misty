...
#'                          variables with 0 and 1 values, i.e., factors and
#'                          character variables are excluded from \code{x} before
#'                          conducting the analysis. Alternatively, an expression
#'                          indicating the variable names in \code{data} e.g.,
#'                          \code{ci.prop(x1, x2, x3, data = dat)}. Note that the
...
ci.prop <- function(..., data = NULL, method = c("wald", "wilson"),
  ...
  ## Grouping, No Split ####
    ...
    if (isTRUE(boot == "none")) {
      ...
      result <- lapply(split(x, f = group), function(y) misty::ci.prop(y, data = NULL, group = NULL, split = NULL, method = method, alternative = alternative, conf.level = conf.level, sort.var = sort.var, digits = digits, check = FALSE, output = FALSE)$result) |>
    ...
    ## Bootstrapping ####
      ...
      result.boot <- lapply(split(x, f = group), function(y) misty::ci.prop(y, data = NULL, group = NULL, split = NULL, seed = seed, boot = boot, R = R, alternative = alternative, conf.level = conf.level, check = FALSE, output = FALSE)) |>
  ...
  ## No Grouping, Split ####
    ...
    if (isTRUE(boot == "none")) {
      ...
      result <- lapply(split(data.frame(x), f = split),
                       function(y) misty::ci.prop(y, data = NULL, group = NULL, split = NULL, method = method, alternative = alternative, conf.level = conf.level, sort.var = sort.var, digits = digits, check = FALSE, output = FALSE)$result)
    ...
    ## Bootstrapping ####
      ...
      result.boot <- lapply(split(x, f = split), function(y) misty::ci.prop(y, data = NULL, group = NULL, split = NULL, seed = seed, boot = boot, R = R, alternative = alternative, conf.level = conf.level, check = FALSE, output = FALSE)) |>
  ...
  ## Grouping, Split ####
    ...
    result <- lapply(split(data.frame(x, group = group), f = split),
                     function(y) misty::ci.prop(y[, -grep("group", names(y))], data = NULL, group = y$group, split = NULL, method = method, alternative = alternative, conf.level = conf.level, sort.var = sort.var, digits = digits,  check = FALSE, output = FALSE)$result)
    ...
    ## Bootstrapping ####
      ...
      result.boot <- lapply(split(data.frame(x, group = group), f = split), function(y) misty::ci.prop(y[, -grep("group", names(y))], data = NULL, group = y$group, split = NULL, seed = seed, boot = boot, R = R, alternative = alternative, conf.level = conf.level, check = FALSE, output = FALSE)) |>
        (\(z) list(boot.sample = data.frame(split = rep(names(z), each = R*unique(unlist(lapply(z, function(q) nrow(q$result))))), do.call("rbind", lapply(z, function(w) w$boot)), row.names = NULL), result = lapply(z, function(w) w$result)))()
