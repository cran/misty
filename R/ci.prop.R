...
#' ci.prop(dat$x1)
#'
#' # Two-Sided 95% Confidence Interval for x1 using Wald method
...
#'
#' # Two-Sided 95% Confidence Interval, print results with 4 digits
...
ci.prop <- function(x, method = c("wald", "wilson"), alternative = c("two.sided", "less", "greater"),
  ...
  if (missing(x)) {
    ...
    stop("Please specify a numeric vector, matrix or data frame with numeric variables with 0 and 1 values for the argument 'x'.",
         call. = FALSE)
  ...
  if (isTRUE(check)) {
    ...
    #......
    # Check input 'method'
    if (!all(method %in%  c("wald", "wilson"))) {
  ...
  if (all(c("wald", "wilson") %in% method)) { method <- "wilson" }
